...
**当前实现状态：mitigated / fail-loud phase 1（非完整 fixed）。**
- warn/error 已实现；**apply 尚未实现**。
- native 角色 prompt **仍可能未送达**。
- 后续能力矩阵 / 安全 apply 保持开放；不得关闭上游 #2853 为完整修复。
...
| **error** | `OMNIGENT_STRICT_PROMPT` 或关键角色要求时 **fail-fast** |
...
2. 实现共同 **fail-loud**：非空 system_prompt + native 且未 apply → warning；`OMNIGENT_STRICT_PROMPT` → error。
