...
from omnigent._platform import (
    IS_WINDOWS,
    inspect_platform,
    native_harness_guidance,
    resolve_repo_symlink,
)
...
def _reject_native_on_windows(harness: str) -> None:
    ...
    """
    if IS_WINDOWS:
        raise click.ClickException(native_harness_guidance(harness))


@cli.command("platform-info")
@click.option("--json", "as_json", is_flag=True, help="以 JSON 输出平台能力，便于安装脚本和 Agent 读取。")
def platform_info(as_json: bool) -> None:
    """显示当前系统可用的 Omnigent 中文版运行模式。"""
    capabilities = inspect_platform()
    if as_json:
        click.echo(json.dumps(capabilities.as_dict(), ensure_ascii=False, sort_keys=True))
        return

    click.echo(f"系统: {capabilities.family}")
    click.echo(f"WSL2: {'是' if capabilities.is_wsl else '否'}")
    click.echo(f"推荐模式: {capabilities.recommended_mode}")
    click.echo(f"原生 tmux/PTY: {'可用' if capabilities.native_tmux_supported else '不可用'}")
    click.echo(f"沙箱后端: {capabilities.sandbox_backend}")
    if capabilities.requires_bubblewrap:
        click.echo("前置工具: tmux、bubblewrap、Node.js 22+、uv")
    elif capabilities.family == "macos":
        click.echo("前置工具: tmux、Node.js 22+、uv（macOS 使用 seatbelt，不需要 bubblewrap）")
    else:
        click.echo("Windows 原生模式: 网页/SDK 可用；完整多 CLI 请使用 WSL2。")
