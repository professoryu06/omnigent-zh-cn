...
def _materialize_override_bundle(source: Path, overrides: ChatOverrides) -> Path:
    ...
    try:
        if source.is_file():
            ...
            # ``Path('.')`` has an empty ``name``. Use the resolved directory
            # name so ``omnigent run .`` copies into a child of ``tmpdir``
            # instead of trying to copy onto the pre-created temp directory.
            target_dir = tmpdir / source.resolve().name
