...
_DEFAULT_KIMI_LAUNCH_ARGS = ("--yolo",)
_SESSION_LABELS = {
...
def _effective_kimi_launch_args(kimi_args: Sequence[str]) -> list[str]:
    """Return persisted Kimi arguments with the local unattended default.

    Native Omnigent sessions run inside a runner-owned tmux pane.  A Kimi
    permission prompt in that pane blocks the web conversation because there
    is no operator at the terminal to answer it.  ``--yolo`` removes Kimi's
    duplicate TUI prompt while the Omnigent PreToolUse policy hook remains the
    authoritative deny gate.  Callers can still opt into Kimi's selective
    prompt mode explicitly with ``--auto``.
    """
    args = list(kimi_args)
    if "--yolo" in args or "--auto" in args:
        return args
    return [*_DEFAULT_KIMI_LAUNCH_ARGS, *args]
...
async def _prepare_kimi_terminal_via_daemon(
    *,
    ...
    """
    persist_args = _effective_kimi_launch_args(kimi_args)
