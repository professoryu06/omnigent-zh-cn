...
class PlatformCapabilities:
    """Runtime capabilities relevant to the Chinese distribution's launch guidance."""

    family: str
    is_wsl: bool
    native_tmux_supported: bool
    sandbox_backend: str
    requires_bubblewrap: bool
    recommended_mode: str

    def as_dict(self) -> dict[str, str | bool]:
        """Return a stable JSON-friendly representation for CLI and automation use."""
        return asdict(self)
...
def inspect_platform(
    *,
    ...
    """Classify the host without probing credentials or installed CLI tools.

    Windows supports the server, web UI, and SDK harnesses, but not native
    tmux/PTY harnesses. WSL is Linux from Python's perspective and is the
    supported full-mode path for Windows users.
    """
    effective_os_name = os.name if os_name is None else os_name
    effective_platform = sys.platform if sys_platform is None else sys_platform
    effective_release = platform.release() if release is None else release

    if effective_os_name == "nt":
        return PlatformCapabilities(
            family="windows",
            is_wsl=False,
            native_tmux_supported=False,
            sandbox_backend="windows_jobobject",
            requires_bubblewrap=False,
            recommended_mode="sdk_or_web",
        )
    ...
    if effective_platform == "darwin":
        return PlatformCapabilities(
            family="macos",
            is_wsl=False,
            native_tmux_supported=True,
            sandbox_backend="darwin_seatbelt",
            requires_bubblewrap=False,
            recommended_mode="native_cli",
        )

    is_wsl = effective_platform.startswith("linux") and (
        "microsoft" in effective_release.lower() or bool(os.environ.get("WSL_DISTRO_NAME"))
    )
    return PlatformCapabilities(
        family="linux",
        is_wsl=is_wsl,
        ...
        requires_bubblewrap=True,
        recommended_mode="native_cli",
    )


def native_harness_guidance(harness: str, capabilities: PlatformCapabilities | None = None) -> str:
    """Explain the supported recovery path when a native harness cannot launch."""
    active = inspect_platform() if capabilities is None else capabilities
    if active.native_tmux_supported:
        return ""
    return (
        f"`omnigent-zh {harness}` 需要 tmux/PTY，Windows 原生模式不支持该运行方式。\n"
        "Windows 原生模式可使用网页和 SDK harness：`omnigent-zh run <agent.yaml>`。\n"
        "需要 Claude、Kimi、Qwen 或 Hermes 的完整多 CLI/tmux 模式时，请在 WSL2 中安装并运行 "
        "Omnigent 中文版。"
    )
