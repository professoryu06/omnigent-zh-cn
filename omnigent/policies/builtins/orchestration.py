...
def deny_subagent_cancellation(
    *,
    reason: str = (
        "Sub-agent cancellation is reserved for a human operator. Collect the "
        "child result through the inbox, or let the user stop the session from "
        "the UI."
    ),
) -> Callable[[_Json], _Json]:
    """Factory: prevent an orchestrator from cancelling its own workers.

    An LLM can mistake a slow or silent worker for a failed worker. In a
    long-running team this destroys the very evidence needed to distinguish a
    harness failure from normal model latency. This guard keeps
    ``sys_cancel_task`` available to the human-facing UI while denying only the
    orchestrator's tool call.

    :param reason: Human-readable explanation returned with the denial.
    :returns: An evaluator that denies ``sys_cancel_task`` and abstains from
        all other policy events.
    """

    def _evaluate(event: _Json) -> _Json:
        if _tool_call(event, {"sys_cancel_task"}) is None:
            return _ALLOW
