...
def test_deny_subagent_cancellation_only_blocks_orchestrator_cancel() -> None:
    """The long-running team guard must block only LLM-originated cancellation."""
    evaluate = deny_subagent_cancellation()

    denied = evaluate(_tool_call("sys_cancel_task", task_id="conv_child"))
    assert _result(denied) == "DENY"
    assert "human operator" in denied["reason"]
