...
def test_materialize_override_bundle_accepts_current_directory(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """A relative ``.`` bundle gets a real child directory in the temp copy."""
    import yaml as _yaml

    agent_dir = tmp_path / "pilot"
    agent_dir.mkdir()
    (agent_dir / "config.yaml").write_text(
        "spec_version: 1\n"
        "name: pilot\n"
    ...
    materialized = _materialize_override_bundle(Path("."), ChatOverrides(system_prompt="be terse"))
    try:
        assert materialized.name == "pilot"
        rewritten = _yaml.safe_load((materialized / "config.yaml").read_text())
        assert rewritten["prompt"] == "be terse"
    finally:
        _cleanup_materialized_override_bundle(materialized)
