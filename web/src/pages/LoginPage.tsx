...
/** Render a translated string that may contain code identifiers like `whoami` in a monospace tag. */
function renderWithCode(text: string, codePattern = "whoami", codeClassName = "font-mono") {
  const idx = text.indexOf(codePattern);
  if (idx < 0) return text;
  return (
    <>
      {text.slice(0, idx)}
      <code className={codeClassName}>{codePattern}</code>
      {text.slice(idx + codePattern.length)}
    </>
  );
}
    ...
    >
      <div className="w-full max-w-sm space-y-6">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              {renderWithCode(t("auth.usernameHint"))}
            </p>
          </div>
        ...
        <p className="text-center text-xs text-muted-foreground">
          {renderWithCode(t("auth.adminCredentialsHint"), "~/.omnigent/admin-credentials", "rounded bg-muted px-1 py-0.5 font-mono")}
        </p>
      </div>
    </div>
  );
