...
describe("PoliciesPage zh-CN", () => {
  ...
  it("renders the heading 全局策略 and empty state when locale is zh-CN", async () => {
    renderZhCN();

    expect(await screen.findByText("全局策略")).toBeInTheDocument();
    expect(screen.getByText("未配置全局策略。添加一条以应用于所有会话。")).toBeInTheDocument();
  });

  it("shows zh-CN add dialog title and remove dialog text", async () => {
    vi.mocked(policies.usePolicyRegistry).mockReturnValue({
      data: [
        {
          handler: "omnigent.policies.block_canada",
    ...
    renderZhCN();
    await screen.findByText("未配置全局策略。添加一条以应用于所有会话。");

    fireEvent.click(screen.getByRole("button", { name: "添加策略" }));
    const dialog = await screen.findByRole("dialog");
    expect(within(dialog).getByText("添加全局策略")).toBeInTheDocument();
    expect(within(dialog).getByText("Block Canada")).toBeInTheDocument();

    expect(translate("policies.permissionDenied")).toBe("您没有管理全局策略的权限。");
  });
});
