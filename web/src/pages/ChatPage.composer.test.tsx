...
describe("Composer placeholder", () => {
  ...
  it("shows Chinese placeholder and aria-label when locale is zh-CN", () => {
    setLocale("zh-CN");
    render(<Composer {...composerProps({})} />);
    const input = screen.getByLabelText("向智能体发送消息") as HTMLTextAreaElement;
    expect(input.placeholder).toBe("向智能体提问…");
    setLocale("en");
  });
});
...
// P4D — zh-CN real-UI assertions for ChatPage composer
describe("Composer zh-CN", () => {
  afterEach(() => {
    cleanup();
    vi.restoreAllMocks();
    setLocale("en");
    useChatStore.setState({ selectedEffort: null, selectedModel: null, sessionModelOverride: null, llmModel: null, codexPlanMode: false });
    useChatStore.setState({ conversationId: null });
  });

  it("shows Chinese Plan button label in composer toolbar", () => {
    setLocale("zh-CN");
    renderWithTooltips(
      <Composer {...composerProps({ showCodexPlanMode: true })} />,
    );
    // Plan button shows Chinese label
    const planBtn = screen.getByRole("button", { name: "进入计划模式" });
    expect(planBtn).toBeInTheDocument();
    expect(within(planBtn).getByText("计划")).toBeInTheDocument();
  });

  it("shows Chinese plan mode label in composer status line", () => {
    setLocale("zh-CN");
    useChatStore.setState({
      conversationId: "conv_test",
      codexPlanMode: true,
    ...
    renderWithTooltips(
      <Composer
        {...composerProps({
          showCodexPlanMode: true,
          showCodexGoal: false,
        })}
      />,
    );
    expect(screen.getByTestId("composer-plan-mode")).toHaveTextContent("计划模式");
  });

  it("shows Chinese Drag-and-drop overlay when dragging a file over the composer", () => {
    setLocale("zh-CN");
    const { container } = renderWithTooltips(<Composer {...composerProps()} />);
    // The drag handlers are on a div inside the form; fire dragEnter on that div
    const dropZone = container.querySelector('div[class*="rounded-2xl"]');
    expect(dropZone).not.toBeNull();
    // Must include stopPropagation + types in mocked event object
  ...
  it("opens the AgentPicker and shows Chinese section headers", async () => {
    setLocale("zh-CN");
    const agents = [{ id: "a1", name: "claude", display_name: "Claude SDK", description: null, harness: "claude-sdk", kind: "builtin" as const }];
    renderWithTooltips(
      <Composer
        {...composerProps({
          agents,
    ...
    // Open the picker via /model on the textarea
    ...
    // The picker section header text is visible
