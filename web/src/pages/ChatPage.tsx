...
import { untitledConversationLabel } from "@/shell/sidebarNav";
...
export function ChatPage() {
  ...
  const subAgentTabTitle =
    activeSession?.parentSessionId != null
      ...
      : null;
  useEffect(() => {
    const fallback = urlConvId ? untitledConversationLabel() : "Omnigent";
                ...
                <ConversationEmptyState>
                  <div className="space-y-1.5">
                    ...
                    <p className="text-muted-foreground text-base">
                      {agentsError
                        ? t("chat.failedToLoadAgents", { error: agentsError instanceof Error ? agentsError.message : String(agentsError) })
                        : t("chat.emptyState")}
                    </p>
                  </div>
                </ConversationEmptyState>
  ...
  return (
    <div className="flex flex-1 items-center justify-center px-6">
      <div className="flex max-w-md flex-col items-center gap-3 text-center">
        <h1 className="font-medium text-foreground text-lg">{t("chat.conversationNotFound")}</h1>
        <p className="text-muted-foreground text-sm">
          {t("chat.sessionLoadFailed", { conversationId, reason: error.message })}
        </p>
  ...
  return (
    ...
    >
      {/* The single announced string. Held stable at "Working…" so the rotating
          ...
          owns the visuals). */}
      {show && <span className="sr-only">{t("chat.working")}</span>}
  ...
  return (
    ...
    >
      <Button
        ...
        aria-label={t("chat.jumpToFirstMessage")}
      ...
      >
        ...
        {jumping ? t("chat.loadingHistory") : t("chat.jumpToTop")}
      </Button>
    </div>
  );
}
 ...
 */
export const WORKING_MESSAGE_KEYS = [
  "chat.working",
  "chat.cooking",
  "chat.crunching",
  "chat.tinkering",
 ...
 * through `WORKING_MESSAGE_KEYS` by wall-clock `tick`.
 */
export function workingIndicatorLabel(
  bgCount: number,
  tick = 0,
  t: (key: string, values?: Record<string, string | number>) => string = translate,
): string {
  if (bgCount > 0) {
    return t(bgCount === 1 ? "chat.backgroundTaskRunning" : "chat.backgroundTasksRunning", {
      count: bgCount,
    });
  }
  return t(WORKING_MESSAGE_KEYS[tick % WORKING_MESSAGE_KEYS.length]!);
}
 ...
 * error band.
 */
const SANDBOX_STAGE_KEYS: Record<string, string | undefined> = {
  provisioning: "chat.provisioningSandbox",
  cloning: "chat.cloningRepository",
  starting: "chat.connectingHost",
  connecting: "chat.startingAgent",
};
...
export function SandboxFailedIndicator({ status }: { status: SandboxStatus }) {
  ...
  return (
    ...
    >
      ...
      <span>{status.error ? t("chat.sandboxLaunchFailedWithError", { error: status.error }) : t("chat.sandboxLaunchFailed")}</span>
    </div>
  );
}
      ...
      >
        ...
        <span>
          {liveness.kind === "host_offline"
            ? t("chat.hostOfflineReconnect")
            : t("chat.agentDisconnectedReconnect")}
        </span>
      </button>
    );
  }
...
export function RunnerStartingIndicator({ variant }: { variant: "hero" | "row" }) {
  ...
  const sandboxStageKey =
    sandboxStatus !== null && sandboxStatus.stage !== "failed"
      ? SANDBOX_STAGE_KEYS[sandboxStatus.stage]
      : undefined;
  const sandboxLabel = sandboxStageKey === undefined ? undefined : t(sandboxStageKey);
  ...
  const line = sandboxLabel !== undefined ? `${sandboxLabel}…` : t("chat.startingUp");
  ...
  if (variant === "hero") {
    return (
      <ConversationEmptyState
        ...
        title={sandboxLabel !== undefined ? `${sandboxLabel}…` : t("chat.startingUp")}
        description={
          sandboxLabel !== undefined
            ? t("chat.sandboxSetupHint")
            : t("chat.startupHint")
        }
      />
    );
...
export function mcpStartingLine(starting: string[], total: number): string {
  if (total === 1 && starting.length === 1) {
    return translate("chat.startingMcpServer", { name: starting[0] });
  }
  const shown = starting.slice(0, MCP_STARTING_NAMES_SHOWN);
  if (starting.length > MCP_STARTING_NAMES_SHOWN) shown.push("…");
  return translate("chat.startingMcpServers", { remaining: total - starting.length, total, names: shown.join(", ") });
}
...
export function mcpSettledNames(names: string[]): string {
  ...
  return `${shown.join(", ")}, ${translate("chat.andNMore", { count: names.length - MCP_SETTLED_NAMES_SHOWN })}`;
}
...
export function McpStartupIndicator() {
  ...
  const parts: string[] = [];
  if (failed.length > 0) parts.push(t("chat.mcpFailedWithNames", { names: mcpSettledNames(failed) }));
  if (cancelled.length > 0) parts.push(t("chat.mcpCancelledWithNames", { names: mcpSettledNames(cancelled) }));
  return (
    <Message from="assistant" data-testid="mcp-startup-indicator" role="status">
      <MessageContent>
        <span className="flex items-center gap-2 text-muted-foreground text-sm">
          <AlertTriangleIcon className="size-4 shrink-0" aria-hidden />
          {t("chat.mcpStartupIncomplete", { detail: parts.join("; ") })}
        </span>
      </MessageContent>
    </Message>
  );
      ...
      >
        <div className="flex items-center gap-0.5">
          ...
          <button
            ...
            title={terminalStartingUp ? t("chat.terminalStartingUp") : undefined}
  ...
  const handleCopy = useCallback(() => {
    ...
    copyText(text).then(
      () => {
        ...
        if (isMobile) {
          showToast(<span className="text-sm">{t("chat.copiedToClipboard")}</span>, { duration: 1500 });
        }
      },
      (error) => {
  ...
  return (
    <>
      ...
      {bubble.lifecycle === "failed" && (
        <p className="text-destructive text-xs">{t("chat.errorLabel")}: {bubble.error}</p>
      )}
    </>
  );
}
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <span
          ...
          aria-label={t("chat.contextUsedPercent", { percent: usedPct })}
        >
      ...
      <TooltipContent side="top" className="max-w-44 text-center text-xs">
        <p className="tabular-nums">{t("chat.contextUsedPercentTooltip", { percent: usedPct })}</p>
      </TooltipContent>
    </Tooltip>
  );
}
...
function formatStatusEffortLabel(effort: string | null, raw = false): string | null {
  ...
  return effort.toLowerCase() === "xhigh" ? translate("chat.effortXHigh") : formatEffortLabel(effort);
}
...
export function subAgentComposerLabel(
  ...
  return title ?? session.subAgentName ?? session.agentName ?? translate("chat.subAgentFallback");
}
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 truncate">
        {t("chat.chattingWithSubAgentPrefix")}<strong className="font-semibold">{label}</strong>
      </span>
    </div>
  );
}
...
export function Composer({
  ...
  const toggleCodexPlanMode = async () => {
    ...
    try {
      ...
      setCommandError(t("chat.planModeError", { action: codexPlanMode ? t("chat.exitPlanMode") : t("chat.enterPlanMode"), message }));
    } finally {
  ...
  const executeSlashCommand = (cmd: string, arg: string): boolean => {
    switch (cmd) {
      case "/compact":
        if (!showCompact) {
          setCommandError(t("chat.compactNotSupported"));
          return true;
        }
        dirtyRef.current = true;
        setValue("");
        ...
        void useChatStore
          ...
          .compact()
          .catch((err: unknown) => {
            setCommandError(err instanceof Error ? err.message : t("chat.compactFailed"));
          });
        return true;
      case "/effort": {
        ...
        if (!arg || !valid.includes(arg.toLowerCase())) {
          setCommandError(t("chat.effortUsage", { valid: valid.join(" | ") }));
          return true;
        }
        ...
        void useChatStore
          ...
          .setEffort(level)
          .catch((err: unknown) => {
            setCommandError(err instanceof Error ? err.message : t("chat.failedToSetEffort"));
          });
        return true;
      }
      case "/model": {
        ...
        if (!target) {
          ...
          const current = sessionModelOverride
            ? t("chat.modelOverride", { model: sessionModelOverride })
            : (llmModel ?? t("chat.agentDefaultModel"));
          setCommandError(t("chat.modelUsage", { current }));
          return true;
        }
        ...
        void useChatStore
          ...
          .catch((err: unknown) => {
            setCommandError(err instanceof Error ? err.message : t("chat.failedToSetModel"));
          });
        return true;
      }
      case "/context": {
        ...
        const lines: string[] = [];
        if (sessionModelOverride) lines.push(t("chat.modelOverride", { model: sessionModelOverride }));
        else if (llmModel) lines.push(t("chat.contextModel", { model: llmModel }));
        ...
        if (tokensUsed != null && contextWindow != null && contextWindow > 0) {
          ...
          lines.push(t("chat.contextWindowSizeUnknown"));
        } else {
          lines.push(t("chat.noUsageData"));
        }
        lines.push(t("chat.itemsInContext", { count: blocks.length }));
      ...
      default:
        setCommandError(
          t("chat.unknownCommand", { cmd, available: Object.keys(slashCommands).join(", ") }),
        );
        return false;
    }
  };
      ...
      >
        {isDragActive && (
          <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center rounded-2xl bg-card/80">
            <span className="text-sm font-medium text-ring">{t("chat.dropFilesHere")}</span>
          </div>
        )}
        ...
        <div className="relative">
          ...
          <textarea
            ...
            aria-label={t("chat.messageTheAgent")}
            placeholder={
              readOnlyReason !== null
                ? readOnlyReason
                : isReadOnly
                  ? t("chat.readOnly")
                  : unreachable
                    ? t("chat.sessionOffline")
                    : hasPendingElicitation
                      ? t("chat.pendingRequest")
                      : disabled
          ...
          <div className="flex flex-wrap gap-1.5 px-4 pb-2">
            {files.map((file, i) => (
              ...
              >
                ...
                <span className="max-w-[140px] truncate">{file.name || t("chat.unknownFilePath")}</span>
                <button
                  type="button"
                  onClick={() => removeFile(i)}
                  className="ml-0.5 rounded-full hover:text-foreground"
                  aria-label={t("chat.removeFile", { name: file.name || t("chat.unknownFilePath") })}
                >
            ...
            {mentionedItems.map((item, i) => (
              ...
              >
                ...
                <button
                  ...
                  className="ml-0.5 rounded-full hover:text-foreground"
                  aria-label={t("chat.removeFile", { name: item.path })}
                >
                  <XIcon className="size-3" />
                </button>
              </span>
        ...
        <div className="flex items-center justify-between gap-2 px-2 pb-2">
          ...
          <div className="flex min-w-0 items-center gap-0.5">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    aria-label={codexPlanMode ? t("chat.exitPlanMode") : t("chat.enterPlanMode")}
                ...
                <TooltipContent>
                  {codexPlanMode ? t("chat.exitPlanMode") : t("chat.enterPlanMode")}
                </TooltipContent>
              </Tooltip>
            )}
            {showCodexGoal && (
            ...
            >
              ...
              <span className="sr-only">{showInterruptButton ? t("chat.interrupt") : t("chat.send")}</span>
            </Button>
          </div>
        </div>
      </div>
...
function AgentPicker({
  ...
  if (isLoading) {
    triggerContent = t("chat.loading");
  } else if (!hasAgents) {
    triggerContent = t("chat.noAgents");
  } else if (modelLabel) {
    triggerContent = (
      <>
    ...
    triggerContent = kiroLaunchFallbackLabel ?? agentDisplayName ?? t("chat.model");
  } else {
    return null;
  }
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      <DropdownMenuContent align="start" className="min-w-64 p-1">
        {showAgents && (
          <>
            <PickerSectionHeader>{t("chat.agents")}</PickerSectionHeader>
            {agents?.map((a) => (
              <DropdownMenuItem
                key={a.id}
        ...
        {modelOptions.length > 0 && (
          <>
            ...
            <PickerSectionHeader>{t("chat.models")}</PickerSectionHeader>
            {modelOptions.map((m) => {
        ...
        {showEffort && (
          <>
            ...
            <PickerSectionHeader>{t("chat.effort")}</PickerSectionHeader>
