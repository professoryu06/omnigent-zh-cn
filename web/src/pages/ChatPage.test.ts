...
describe("workingIndicatorLabel", () => {
  ...
  it("pins the first rotation message to 'Working…'", () => {
    ...
    expect(WORKING_MESSAGE_KEYS[0]).toBe("chat.working");
    expect(workingIndicatorLabel(0, 0)).toBe("Working…");
  });

  it("rotates through the message pool by tick", () => {
    expect(workingIndicatorLabel(0, 1)).toBe("Cooking…");
    expect(workingIndicatorLabel(0, 2)).toBe("Crunching…");
  });

  it("wraps the rotation modulo the pool length", () => {
    expect(workingIndicatorLabel(0, WORKING_MESSAGE_KEYS.length)).toBe("Working…");
    expect(workingIndicatorLabel(0, WORKING_MESSAGE_KEYS.length + 1)).toBe("Cooking…");
  });
  ...
  it("reads the active locale when formatting a working label", () => {
    setLocale("en");
    expect(workingIndicatorLabel(0)).toBe("Working…");

    setLocale("zh-CN");
    expect(workingIndicatorLabel(0)).toBe("工作中…");
    expect(workingIndicatorLabel(2)).toBe("仍有 2 个后台任务在运行");
