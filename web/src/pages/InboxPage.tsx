  ...
  return (
    <PageScroll contentClassName="px-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-semibold">{t("inbox.heading")}</h1>
        {(items.length > 0 || commentInbox.items.length > 0) && (
          <span className="text-sm text-muted-foreground">
            {[
              items.length > 0 &&
                t(items.length === 1 ? "inbox.approvalSingular" : "inbox.approvalPlural", {
                  count: items.length,
                }),
              commentInbox.items.length > 0 &&
                t(
                  commentInbox.items.length === 1 ? "inbox.commentSingular" : "inbox.commentPlural",
                  { count: commentInbox.items.length },
                ),
            ]
              .filter(Boolean)
              .join(" · ")}{" "}
      ...
      {failedSessionCount > 0 && (
        ...
        >
          ...
          <span className="flex-1">
            {t(failedSessionCount === 1 ? "inbox.loadErrorSingular" : "inbox.loadErrorPlural", {
      ...
      <div className="flex flex-col gap-4">
        ...
        {commentInbox.items.map((item) => {
          ...
          const author = comment.created_by ?? t("comment.you");
