...
describe("MembersPage zh-CN", () => {
  ...
  it("renders the heading 成员 and table header 用户名 when locale is zh-CN", async () => {
    vi.mocked(accountsApi.listUsers).mockResolvedValue([user({ id: "bob" })]);
    renderZhCN();
  ...
  it("shows zh-CN invite dialog title and create-invite button", async () => {
    renderZhCN();
    await screen.findByText("暂无成员。");

    fireEvent.click(screen.getByRole("button", { name: "邀请成员" }));
    const dialog = await screen.findByRole("dialog");
    expect(within(dialog).getByText("邀请成员")).toBeInTheDocument();
    expect(within(dialog).getByRole("button", { name: "创建邀请" })).toBeInTheDocument();
  });

  it("shows the localized load error when the members request fails", async () => {
    vi.mocked(accountsApi.listUsers).mockResolvedValue(null);
    renderZhCN();

    expect(
      await screen.findByText("无法加载成员。您可能没有管理员权限，或者服务器无法访问。"),
    ).toBeInTheDocument();
  });
});
