...
describe("BrowserPane zh-CN", () => {
  function renderCN(conversationId: string) {
    return render(
      <I18nProvider>
  ...
  it("renders Chinese labels for all toolbar buttons and address bar", async () => {
    setLocale("zh-CN");
    renderCN("conv_cn1");

    await screen.findByRole("textbox", { name: "地址栏" });
    ...
    expect(screen.getByRole("button", { name: "切换开发者工具" })).toBeInTheDocument();
  });

  it("shows the Chinese empty-state hint when no view is active", async () => {
    setLocale("zh-CN");
    renderCN("conv_cn2");
    await screen.findByRole("textbox", { name: "地址栏" });
    expect(
      screen.getByText("在上方输入网址开始——智能体也会在这里打开页面。"),
    ).toBeInTheDocument();
  });

  it("renders design-mode button with Chinese label in its initial state", async () => {
    setLocale("zh-CN");
    renderCN("conv_cn3");
    await screen.findByRole("textbox", { name: "地址栏" });
  ...
  it("renders the URL input with Chinese placeholder text", async () => {
    setLocale("zh-CN");
    renderCN("conv_cn4");
    const urlInput = await screen.findByRole("textbox", { name: "地址栏" });
    expect(urlInput).toHaveAttribute("placeholder", "输入网址");
  });
});
