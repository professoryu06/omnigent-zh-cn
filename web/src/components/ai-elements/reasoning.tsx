...
const defaultGetThinkingMessage = (t: ReturnType<typeof import("@/i18n").useTranslation>["t"]) => (isStreaming: boolean, duration?: number) => {
  if (isStreaming) {
  ...
  if (duration === undefined) {
    return <span>{t("reasoning.thoughtFewSeconds")}</span>;
  }
...
export const ReasoningTrigger = memo(
  ({
    ...
    const thinkingMessage = getThinkingMessage ?? defaultGetThinkingMessage(t);
