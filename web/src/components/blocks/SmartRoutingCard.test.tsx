...
describe("zh-CN", () => {
  ...
  it("shows Chinese labels for the routing card heading", () => {
    setLocale("zh-CN");
    render(<SmartRoutingCard arguments={TWO_TASK_ARGS} output={null} state="input-available" />);
    expect(card()).toHaveTextContent("智能路由");
  });

  it("shows Chinese noun for tasks in the weighing message", () => {
    setLocale("zh-CN");
    render(<SmartRoutingCard arguments={TWO_TASK_ARGS} output={null} state="input-available" />);
    expect(card()).toHaveTextContent("任务");
  });
});
