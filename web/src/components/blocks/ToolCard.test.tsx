...
describe("ToolCard rendering", () => {
  ...
  it("expands to reveal the Parameters panel and output on click", () => {
    ...
    // i18n fallback to key name since tool.* keys not registered in en.ts yet
...
describe("ToolGroupSummary", () => {
  ...
  it("labels the run with a pluralized step count and renders children when expanded", () => {
    ...
    // i18n fallback to key name — tool.* keys are registered by main agent
    ...
    expect(screen.getByText(/See \d+ steps/)).toBeInTheDocument();
    fireEvent.click(container.querySelector<HTMLElement>('[data-slot="collapsible-trigger"]')!);
  ...
  it("uses the singular 'step' for one tool and honors an explicit count override", () => {
    ...
    expect(screen.getByText(/See \d+ step/)).toBeInTheDocument();
...
// zh-CN i18n tests — use setLocale("zh-CN") before render and assert
// on actual Chinese text in the DOM. Restore setLocale("en") in afterEach.
describe("zh-CN", () => {
  afterEach(() => {
    setLocale("en");
  });

  it("shows Chinese labels for Parameters/Output panels and copy buttons", () => {
    setLocale("zh-CN");
    const { container } = renderCard({
      name: "my_tool",
      arguments: { a: 2 },
  ...
  it("shows Chinese Expand/Collapse button text for truncated output", () => {
    setLocale("zh-CN");
    const output = Array.from({ length: 85 }, (_, i) => `line ${i + 1}`).join("\n");
    const { container } = renderCard({
      name: "my_tool",
      arguments: {},
      output,
    ...
    // Initially shows "展开" for collapsed state
    expect(screen.getByText("展开")).toBeInTheDocument();
    expect(screen.getByText(/正在预览输出/)).toBeInTheDocument();

    // Click expand
    fireEvent.click(screen.getByText("展开"));
    // After expand, shows "收起"
    expect(screen.getByText("收起")).toBeInTheDocument();
    expect(screen.getByText(/显示完整输出/)).toBeInTheDocument();
  });

  it("shows Chinese waiting-for-output placeholder", () => {
    setLocale("zh-CN");
    const { container } = renderCard({
      name: "my_tool",
      arguments: {},
  ...
  it("shows Chinese empty-output messages for each state", () => {
    setLocale("zh-CN");
    const { container } = renderCard({
      name: "my_tool",
      arguments: {},
    ...
    const trigger = container.querySelector<HTMLElement>('[data-slot="collapsible-trigger"]')!;
    fireEvent.click(trigger);
    expect(screen.getByText("工具在输出到达前已被取消。")).toBeInTheDocument();
  });

  it("shows Chinese text for ToolGroupSummary step label", () => {
    setLocale("zh-CN");
    function item(id: string, name: string): RenderItem {
      return {
        kind: "tool",
        execution: { callId: id, name, argsSummary: "", arguments: {} },
        output: "ok",
        state: "output-available",
        startedAt: null,
        duration: 1,
    ...
    render(
      createElement(
        ...
        null,
        createElement(ToolGroupSummary, {
          tools: [item("t1", "alpha_tool"), item("t2", "beta_tool")],
        }),
      ),
    );
