...
describe("zh-CN", () => {
  ...
  it("shows the hint bar in Chinese on macOS", () => {
    setLocale("zh-CN");
    const hint = selectionHintText(true);
    expect(hint).toBe("按住 ⌥ 并拖拽以选择 · ⌘C 复制");
    setLocale("en");
  });

  it("shows the hint bar in Chinese on Windows/Linux", () => {
    setLocale("zh-CN");
    const hint = selectionHintText(false);
    expect(hint).toBe("按住 Shift 并拖拽以选择 · 右键复制");
    setLocale("en");
  });
});
