...
export function selectionHintText(isMac: boolean): string {
  return isMac
    ? translate("terminal.macHint")
    : translate("terminal.winHint");
}
    ...
    <div className="absolute inset-0 z-[10000] flex items-center justify-center bg-background/85 text-sm text-foreground backdrop-blur-[1px]">
      ...
      {state.kind === "closed" && !reconnectPending && (
        <div className="flex flex-wrap items-center justify-center gap-2 px-3">
          <span>{t("terminal.bridgeClosed", { reason: state.reason })}</span>
          {onResume && (
            <Button
              type="button"
              size="xs"
            ...
            >
              {resumePending ? t("terminal.resuming") : t("terminal.resumeSession")}
            </Button>
          )}
          {resumeError && (
      ...
      {state.kind === "error" && <span>{t("terminal.bridgeError")}</span>}
    </div>
  );
}

function resumeErrorText(error: unknown): string {
  if (error instanceof Error && error.message) return translate("terminal.couldntResume", { message: error.message });
  return translate("terminal.couldntResumeFallback");
}
