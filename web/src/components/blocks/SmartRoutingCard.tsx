...
export function SmartRoutingCard({ arguments: args, output, state }: SmartRoutingCardProps) {
  ...
  const taskNoun = tasks.length === 1 ? t("routing.task") : t("routing.tasks");
  return (
    <Collapsible
      defaultOpen={false}
      className={cn(
      ...
      <div className="flex items-center gap-1.5 text-xs">
        ...
        {judging ? (
          <Shimmer as="span" className="text-xs">
            {t("routing.weighingTasks", { count: tasks.length, noun: taskNoun })}
          </Shimmer>
        ) : (
          <span className="text-muted-foreground">
            {failed ? t("routing.unavailable") : t("routing.sizedTasks", { count: recommendations!.size, noun: taskNoun })}
          </span>
        )}
        {prettyOutput !== null && (
          <CollapsibleTrigger
            className="ml-auto cursor-pointer rounded p-0.5 text-muted-foreground hover:text-foreground"
            aria-label={t("routing.showRawResponse")}
          ...
          <CodeBlock code={prettyOutput} language="json">
            <CodeBlockHeader>
              <CodeBlockTitle className="min-w-0">
                <span className="truncate font-medium uppercase tracking-wide">{t("routing.response")}</span>
              </CodeBlockTitle>
            </CodeBlockHeader>
          </CodeBlock>
        </CollapsibleContent>
