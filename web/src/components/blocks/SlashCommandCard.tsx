...
export function SlashCommandCard({ kind, name, arguments: args, output }: SlashCommandCardProps) {
  ...
  const { labelKey, Icon, iconClass } = KIND_STYLES[kind];
  const label = t(labelKey);
  // Tooltip keeps the title legible after truncation of long names.
  const tooltip = useMemo(
    () => (hasArgs ? `${label} ${name} ${args}` : `${label} ${name}`),
    [hasArgs, label, name, args],
  );
  ...
  const trigger = (
    ...
    >
      ...
      <span className="min-w-0 flex-1 truncate">
        <span className="font-semibold text-foreground">{label}</span> <span>{name}</span>
        {hasArgs && (
          <>
            {" "}
    ...
    <CodeBlock code={args} language="bash">
      <CodeBlockHeader>
        <CodeBlockTitle className="min-w-0">
          <span className="truncate font-medium uppercase tracking-wide">{t("slashCommand.arguments")}</span>
        </CodeBlockTitle>
      </CodeBlockHeader>
    </CodeBlock>
  );
...
function OutputPanel({ output }: { output: string }) {
  ...
  return (
    <CodeBlock code={output} language="bash">
      <CodeBlockHeader>
        <CodeBlockTitle className="min-w-0">
          <span className="truncate font-medium uppercase tracking-wide">{t("slashCommand.output")}</span>
        </CodeBlockTitle>
      </CodeBlockHeader>
    </CodeBlock>
  );
