  ...
  return (
    <Collapsible defaultOpen={false} className="group not-prose w-full">
      ...
      <CollapsibleContent className="mt-1 ml-2 space-y-2 border-l pl-3 py-1 data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:animate-out data-[state=open]:animate-in">
        <CodePanel
          title={t("tool.parameters")}
          text={inputJson}
          copyText={inputJson}
          copyLabel={t("tool.copyParameters")}
        />
...
export function ToolGroupSummary({ tools, count }: { tools: RenderItem[]; count?: number }) {
  ...
  const label = n === 1 ? t("tool.seeStep", { count: n }) : t("tool.seeSteps", { count: n });
  return (
...
function OutputSection({ output }: { output: string }) {
  ...
  return (
    <div className="space-y-2">
      ...
      >
        <CodePanel title={t("tool.output")} text={preview.text} copyText={output} copyLabel={t("tool.copyOutput")} />
      ...
      {canExpand && (
        <div className="flex flex-col gap-2 rounded-md border bg-muted/30 px-3 py-2 text-muted-foreground text-xs sm:flex-row sm:items-center sm:justify-between">
          <span className="min-w-0">
            {isExpanded ? t("tool.showingFullOutput") : t("tool.previewingOutput")} (
          ...
          >
            ...
            {isExpanded ? t("tool.collapse") : t("tool.expand")}
          </Button>
        </div>
      )}
    </div>
...
function EmptyOutputState({ state }: { state: "output-error" | "cancelled" | "no-output" }) {
  ...
  let message: string;
  if (state === "cancelled") {
    message = t("tool.cancelledBeforeOutput");
  } else if (state === "no-output") {
    message = t("tool.noOutputRecorded");
  } else {
    message = t("tool.didNotReturnOutput");
  }
  return (
...
function formatOutputStats(preview: OutputPreview): string {
  if (!preview.isTruncated) {
    return `${formatCount(preview.lineCount, translate("tool.lineUnit"))} / ${formatCount(preview.charCount, translate("tool.charUnit"))}`;
  }

  const hidden: string[] = [];
  if (preview.hiddenLineCount > 0) {
    hidden.push(`${formatCount(preview.hiddenLineCount, translate("tool.lineUnit"))} ${translate("tool.hidden")}`);
  }
  if (preview.hiddenCharCount > 0) {
    hidden.push(`${formatCount(preview.hiddenCharCount, translate("tool.charUnit"))} ${translate("tool.hidden")}`);
  }

  return `${formatCount(preview.shownLineCount, translate("tool.lineUnit"))} / ${formatCount(
    preview.shownCharCount,
    translate("tool.charUnit"),
  )} ${translate("tool.shown")}; ${hidden.join(", ")}`;
}
