...
describe("zh-CN", () => {
  ...
  it("shows Chinese labels for the routing chip and card", () => {
    setLocale("zh-CN");

    const { unmount } = render(
      <RoutingDecisionChip model="databricks-claude-opus-4-8" applied rationale="x" />,
    );
    expect(screen.getByText(/智能模型路由器/)).toBeInTheDocument();
    unmount();
  ...
  it("shows Chinese for ErrorBanner, CompactionMarker, and PolicyDeniedBanner", () => {
    setLocale("zh-CN");

    const { unmount } = render(<ErrorBanner message="" source="" code="" />);
    expect(screen.getByText("未知错误")).toBeInTheDocument();
    unmount();

    render(<CompactionMarker />);
    expect(screen.getByText("会话已压缩")).toBeInTheDocument();

    cleanup();

    render(<PolicyDeniedBanner reason="测试原因" phase="" />);
    expect(screen.getByText("被策略阻止")).toBeInTheDocument();
  });
  ...
  it("switches from English to Chinese at runtime without remounting", () => {
    ...
    // Initially English
    expect(screen.getByText(/Intelligent model router/)).toBeInTheDocument();

    // Switch locale at runtime
    fireEvent.click(screen.getByText("Switch to Chinese"));

    // Now Chinese without remounting
