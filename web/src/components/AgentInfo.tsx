...
const MODEL_TOKEN_ROWS: ReadonlyArray<{ key: keyof ModelUsage; labelKey: string }> = [
  { key: "inputTokens", labelKey: "agentInfo.input" },
  { key: "outputTokens", labelKey: "agentInfo.output" },
  { key: "cacheReadInputTokens", labelKey: "agentInfo.cacheRead" },
  { key: "cacheCreationInputTokens", labelKey: "agentInfo.cacheWrite" },
  { key: "totalTokens", labelKey: "agentInfo.total" },
];
          ...
          return (
            ...
            >
              ...
              {rows.map((row) => (
                ...
                >
                  <span className="text-muted-foreground/70">{t(row.labelKey)}</span>
              ...
              {usage.totalCostUsd != null && (
                <div className="flex items-baseline justify-between gap-3 pl-2 text-xs">
                  <span className="text-muted-foreground/70">{t("agentInfo.cost")}</span>
                  <span className="tabular-nums text-muted-foreground">
  ...
  return (
    ...
    >
      <DialogContent className="max-h-[80vh] overflow-y-auto sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{t("agentInfo.addPolicy")}</DialogTitle>
          <DialogDescription>{t("agentInfo.addPolicyDesc")}</DialogDescription>
        </DialogHeader>
        <div className="min-w-0 space-y-3 pt-1">
              ...
              return (
                <>
                  <input
                    ...
                    placeholder={t("agentInfo.filterPolicies")}
                  ...
                  <div className="flex max-h-52 flex-col divide-y divide-border overflow-y-auto rounded border border-border">
                    ...
                    {filtered.length === 0 && (
                      <p className="py-2 text-center text-xs text-muted-foreground">
                        {available.length === 0
                          ? t("agentInfo.allPoliciesApplied")
                          : t("agentInfo.noPoliciesMatch")}
                      </p>
                    )}
                  </div>
                </>
          ...
          {entry && (
            <div>
              <label className="flex items-center gap-1 text-xs text-muted-foreground">
                <span className="font-medium text-foreground">{t("agentInfo.name")}</span>
              </label>
              <input
                type="text"
                value={policyName}
                      ...
                      {prop?.type && (
                        <span>
                          ...
                          {prop.type === "array" && prop.items?.enum
                            ? t("agentInfo.multiSelect")
                            : prop.type === "array"
                              ? t("agentInfo.commaSeparated")
                              : prop.type}
                          )
                        </span>
                      )}
                        ...
                        placeholder={
                          prop?.type === "array"
                            ? prop?.default !== undefined
                              ...
                              : t("agentInfo.commaSeparatedValues")
                            : prop?.default !== undefined
                              ? String(prop.default)
                              : ""
                        }
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            >
              {addPolicy.isPending ? t("agentInfo.adding") : t("agentInfo.add")}
            </button>
          </div>
        </div>
      </DialogContent>
...
function validateMcpForm(form: McpFormState): string | null {
  ...
  if (!name) return translate("agentInfo.nameRequired");
  if (!/^[A-Za-z0-9_-][A-Za-z0-9_.-]{0,127}$/.test(name)) {
    return translate("agentInfo.nameInvalid");
  }
  if (form.transport === "http") {
    const url = form.url.trim();
    if (!url) return translate("agentInfo.urlRequired");
    if (!url.startsWith("http://") && !url.startsWith("https://")) {
      return translate("agentInfo.urlInvalid");
    }
  }
  if (form.transport === "stdio" && !form.command.trim()) {
    return translate("agentInfo.commandRequired");
  }
  return null;
}
...
function McpServerManagerDialog({
  ...
  function notifyRestart() {
    onDirty();
    showToast(
      <span className="text-sm">{translate("agentInfo.mcpRestartToast")}</span>,
    );
  }
  ...
  return (
    ...
    >
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{t("agentInfo.manageMcpServers")}</DialogTitle>
          <DialogDescription>{t("agentInfo.manageMcpDesc")}</DialogDescription>
        </DialogHeader>
        {dirty && (
        ...
        <div className="grid gap-4 pt-1 md:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          <div className="flex min-w-0 flex-col gap-1.5">
            ...
            {servers.length > 0 ? (
                ...
                {servers.map((server) => (
                  <div key={server.name} className="flex min-w-0 items-center gap-1.5 px-2 py-2">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="icon-xs"
                      aria-label={t("agentInfo.editServer", { name: server.name })}
                      onClick={() => {
                        setForm(mcpFormFromServer(server));
                        setFormError(null);
                      }}
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="icon-xs"
                      aria-label={t("agentInfo.deleteServer", { name: server.name })}
              ...
              <p className="py-3 text-xs text-muted-foreground">{t("agentInfo.noMcpServers")}</p>
            )}
            {form.originalName && (
              <Button type="button" variant="ghost" size="sm" onClick={resetForm}>
                <PlusIcon className="size-3.5" />
                {t("agentInfo.newServer")}
              </Button>
            )}
          </div>

          <div className="flex min-w-0 flex-col gap-2">
            <SectionLabel>{form.originalName ? t("agentInfo.editServerTitle") : t("agentInfo.newServerTitle")}</SectionLabel>
            ...
            <label className="flex flex-col gap-1 text-xs text-muted-foreground">
              ...
              <Input
                ...
                placeholder={t("agentInfo.descriptionOptional")}
              />
            </label>
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              >
                ...
                {saving ? t("agentInfo.saving") : t("agentInfo.save")}
              </Button>
            </div>
          </div>
        </div>
...
function McpServersSection({
  ...
  return (
    <div className="flex flex-col gap-1.5 py-3">
      <div className="flex items-center justify-between">
        <SectionLabel>{t("agentInfo.tools")}</SectionLabel>
        {canEdit && (
          <button
            type="button"
            onClick={() => setManagerOpen(true)}
            className="rounded p-0.5 hover:bg-muted"
            title={t("agentInfo.manageMcpServersTooltip")}
            aria-label={t("agentInfo.manageMcpServersTooltip")}
          >
      ...
      {servers.length > 0 ? (
        ...
        <p className="text-xs text-muted-foreground">{t("agentInfo.noMcpServers")}</p>
      )}
      {canEdit && (
...
function SessionPoliciesSection({ sessionId }: { sessionId: string }) {
  ...
  return (
    <div className="flex flex-col gap-1.5 py-3">
      <div className="flex items-center justify-between">
        <SectionLabel>{t("agentInfo.policies")}</SectionLabel>
        <button
          type="button"
          onClick={() => setAddOpen(true)}
          className="rounded p-0.5 hover:bg-muted"
          title={t("agentInfo.addPolicyTooltip")}
        >
          <PlusIcon className="size-3 text-muted-foreground" />
        </button>
      </div>
      {userPolicies.length > 0 ? (
        ...
        <p className="text-xs text-muted-foreground">{t("agentInfo.noPoliciesAdded")}</p>
      )}
      <AddPolicyDialog
        sessionId={sessionId}
        registry={registry}
...
export function AgentInfoContent({ agent, sessionId }: AgentInfoProps) {
  ...
  const versionFooter = (() => {
    const server = serverVersion ? `${t("agentInfo.server")} ${serverVersion}` : null;
    const host = hostVersion ? `${t("agentInfo.host")} ${hostVersion}` : null;
    return [server, host].filter(Boolean).join(" · ");
  })();
  ...
  return (
    <div className="flex flex-col divide-y divide-border/50">
      ...
      {sessionId && owner && isSessionShared && (
        <div className="flex flex-col gap-1.5 py-3">
          <SectionLabel>{t("agentInfo.owner")}</SectionLabel>
          <span
          ...
          >
            {owner}
            {owner === viewerId && <span className="ml-1 text-muted-foreground/60">{t("agentInfo.you")}</span>}
          </span>
        </div>
      )}
      {sessionId && (
        <div className="flex flex-col gap-1.5 py-3">
          <SectionLabel>{t("agentInfo.sessionId")}</SectionLabel>
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              variant="ghost"
              size="icon-sm"
              aria-label={sessionIdCopied ? t("agentInfo.copiedSessionId") : t("agentInfo.copySessionId")}
          ...
          <div className="flex flex-col gap-2 py-3">
            {sessionCostUsd != null && (
              <div className="flex items-baseline justify-between gap-3">
                <SectionLabel>{t("agentInfo.sessionCost")}</SectionLabel>
                <span
  ...
  return (
    <Popover>
      <Tooltip>
        <TooltipTrigger asChild>
          <PopoverTrigger asChild>
            <Button
              ...
              variant="ghost"
              size="icon"
              aria-label={t("agentInfo.agentToolsAndPolicies")}
        ...
        <TooltipContent>{t("agentInfo.agentToolsPoliciesHtml")}</TooltipContent>
      </Tooltip>
