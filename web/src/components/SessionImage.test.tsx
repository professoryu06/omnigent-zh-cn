...
describe("SessionImage (embedded, host fetcher present)", () => {
  ...
  it("shows the loading placeholder before the fetch resolves", () => {
    ...
    expect(screen.getByRole("status", { name: /load/i })).toBeInTheDocument();
...
// P5-R — zh-CN loading placeholder assertion
describe("SessionImage zh-CN", () => {
  afterEach(() => setLocale("en"));

  it("renders Chinese loading placeholder accessible name when locale is zh-CN", () => {
    setLocale("zh-CN");
    getOmnigentHostConfig.mockReturnValue({ fetcher: () => {} });
    hostFetch.mockReturnValue(new Promise(() => {}));
    render(<SessionImage path="/p" alt="pic" />);
    expect(screen.getByRole("status", { name: "加载图像中" })).toBeInTheDocument();
  });
});
