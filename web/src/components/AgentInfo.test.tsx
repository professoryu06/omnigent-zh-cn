...
import { I18nProvider, __pinDefaultLocale, setLocale, translate, useTranslation } from "@/i18n";
...
describe("AgentInfo zh-CN i18n", () => {
  beforeEach(() => {
    // Clear any stored locale so translate() defaults to zh-CN
    __pinDefaultLocale(null);
    localStorage.removeItem("omnigent:locale");
  });

  afterEach(() => {
    // Reset to English to avoid leaking into other test suites
    setLocale("en");
    __pinDefaultLocale(null);
  });

  describe("translate() direct key checks", () => {
    it("returns Chinese for session info keys", () => {
      expect(translate("agentInfo.owner")).toBe("所有者");
      expect(translate("agentInfo.sessionId")).toBe("会话 ID");
    });

    it("returns Chinese for copy state keys", () => {
      expect(translate("agentInfo.copySessionId")).toBe("复制会话 ID");
      expect(translate("agentInfo.copiedSessionId")).toBe("已复制会话 ID");
    });

    it("returns Chinese for MCP management key", () => {
      expect(translate("agentInfo.manageMcpServers")).toBe("管理 MCP 服务器");
    });

    it("returns Chinese for MCP empty/restart keys", () => {
      expect(translate("agentInfo.noMcpServers")).toBe("无 MCP 服务器");
      expect(translate("agentInfo.restartCompact")).toBe("重新启动以应用更改");
    });

    it("returns Chinese for session policies keys", () => {
      expect(translate("agentInfo.policies")).toBe("策略");
      expect(translate("agentInfo.addPolicy")).toBe("添加策略");
    });

    it("returns Chinese for usage label keys", () => {
      expect(translate("agentInfo.tokenUsage")).toBe("Token 用量");
      expect(translate("agentInfo.input")).toBe("输入");
    });

    it("returns Chinese for accessibility key", () => {
      expect(translate("agentInfo.agentToolsAndPolicies")).toBe("智能体工具与策略");
    });

    it("returns Chinese for session cost key", () => {
      expect(translate("agentInfo.sessionCost")).toBe("会话成本");
    });
  });

  describe("I18nProvider runtime switch", () => {
    it("switches from English to Chinese at runtime without remounting", () => {
      ...
      function AgentInfoDisplay() {
        const { t } = useTranslation();
        return (
          <div>
            <span data-testid="owner-label">{t("agentInfo.owner")}</span>
            <span data-testid="session-id-label">{t("agentInfo.sessionId")}</span>
          </div>
        );
      }
      ...
      // Assert English text is shown initially
      expect(screen.getByTestId("owner-label")).toHaveTextContent("Owner");
      expect(screen.getByTestId("session-id-label")).toHaveTextContent("Session ID");

      // Click Switch to zh-CN
      fireEvent.click(screen.getByRole("button", { name: "Switch" }));

      // Assert Chinese text WITHOUT remounting
      expect(screen.getByTestId("owner-label")).toHaveTextContent("所有者");
      expect(screen.getByTestId("session-id-label")).toHaveTextContent("会话 ID");
    });
  });
});
