      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            ...
            aria-label={goal ? t("goal.viewGoal") : t("goal.setGoal")}
        ...
        <TooltipContent>{goal ? t("goal.viewGoal") : t("goal.setGoal")}</TooltipContent>
      </Tooltip>
      <CodexGoalDialog
        open={open}
...
export function CodexGoalStatusPill({ goal }: { goal: CodexGoal }) {
  ...
  return (
    ...
    >
      ...
      <span>{t("goal.goalPrefix")}{formatCodexGoalStatus(goal.status)}</span>
    </span>
  );
}
