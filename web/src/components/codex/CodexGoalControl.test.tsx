...
describe("CodexGoalControl", () => {
  ...
  it("shows Chinese button label when locale is zh-CN", () => {
    setLocale("zh-CN");
    render(<TooltipProvider><CodexGoalControl goal={null} conversationId="conv_abc" readOnly={false} onGoalChange={vi.fn()} /></TooltipProvider>);
    expect(screen.getByRole("button", { name: "设置 Codex 目标" })).toBeInTheDocument();
    setLocale("en");
  });
});
