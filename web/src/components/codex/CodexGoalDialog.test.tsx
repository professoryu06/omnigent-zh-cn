...
describe("Codex goal budget parsing", () => {
  ...
  it("shows Chinese labels in the dialog when locale is zh-CN", async () => {
    setLocale("zh-CN");
    vi.mocked(getCodexGoal).mockResolvedValue({ goal: null });
    vi.mocked(setCodexGoal).mockReturnValue(new Promise(() => {}));
    render(
      <CodexGoalDialog open onOpenChange={vi.fn()} readOnly={false} conversationId="conv_abc" goal={null} onGoalChange={vi.fn()} />,
    );
