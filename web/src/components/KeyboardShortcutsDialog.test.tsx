...
import { I18nProvider, translate, useTranslation } from "@/i18n";
...
describe("zh-CN i18n", () => {
  it('translates dialog title "Keyboard shortcuts" to "键盘快捷键"', () => {
    expect(translate("shortcuts.title", {}, "zh-CN")).toBe("键盘快捷键");
  });

  it('translates "Send message" shortcut label to "发送消息"', () => {
    expect(translate("shortcuts.sendMessage", {}, "zh-CN")).toBe("发送消息");
  });
  ...
  it("switches from English to Chinese at runtime without remounting", () => {
    ...
    // Open the dialog via hotkey — renders English header by default (test-setup pins "en").
    toggleViaHotkey();
    expect(screen.getByText("Keyboard shortcuts")).toBeInTheDocument();
    fireEvent.click(screen.getByText("Switch"));
    // After switching to zh-CN without remounting, the translated title appears.
