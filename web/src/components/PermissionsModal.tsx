...
/** Numeric permission level → translation key. Resolve via t() at render time. */
const LEVEL_LABELS: Record<number, string> = {
  1: "permissions.read",
  2: "permissions.edit",
  3: "permissions.manage",
  4: "permissions.owner",
};
...
export function PermissionsModal({ sessionId, open, onOpenChange }: PermissionsModalProps) {
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">{t("permissions.sharingUnavailable")}</DialogTitle>
            <DialogDescription>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">{t("permissions.shareThisSession")}</DialogTitle>
          <DialogDescription>
            {sharingReadOnly
              ? t("permissions.readOnlyDescription")
        ...
        {publicSharingEnabled && (
          <div className="flex items-center justify-between rounded-lg border px-3 py-2">
            <div>
              <p className="text-sm font-medium">{t("permissions.publicAccess")}</p>
              <p className="text-xs text-muted-foreground">{t("permissions.publicAccessDesc")}</p>
            </div>
            <Switch
              checked={isPublic}
        ...
        <div className="min-w-0" data-testid="share-grants">
          {isLoading ? (
            <p className="text-sm text-muted-foreground py-2">{t("permissions.loading")}</p>
          ) : userGrants.length === 0 ? (
            <p className="text-sm text-muted-foreground py-2">{t("permissions.noGrants")}</p>
          ) : (
            <>
              {/* Column headers */}
        ...
        <form onSubmit={handleGrant} className="flex items-end gap-2">
          ...
          <div>
            ...
            <Select value={newLevel} onValueChange={setNewLevel}>
              ...
              <SelectContent>
                <SelectItem value="1">{t("permissions.read")}</SelectItem>
                {/* Read-only sharing caps new grants at view; hide Edit. */}
                {!sharingReadOnly && <SelectItem value="2">{t("permissions.edit")}</SelectItem>}
              </SelectContent>
            </Select>
          </div>
    ...
    <div className="relative">
      ...
      {isOpen && (
        ...
        <div className="absolute left-0 top-full z-50 mt-1 w-96 rounded-lg border bg-popover p-1 text-popover-foreground shadow-md">
          {isLoading ? (
            <div className="py-6 text-center text-sm text-muted-foreground">{t("permissions.searching")}</div>
          ) : suggestions.length === 0 ? (
            <div className="py-6 text-center text-sm text-muted-foreground">{t("permissions.noMatches")}</div>
          ) : (
...
function CopyLinkButton({ sessionId }: { sessionId: string }) {
  ...
  return (
    <Button variant="ghost" size="sm" onClick={handleCopy} className="gap-1.5 text-primary">
      ...
      {copied ? t("permissions.copied") : t("permissions.copyLink")}
    </Button>
  );
}
    ...
    <div className="flex items-center gap-2 rounded-md px-2 py-0.5 hover:bg-muted/50">
      ...
      {fixedLevel ? (
        <span className="flex h-8 w-28 items-center px-3 text-sm text-muted-foreground">
          {t(LEVEL_LABELS[permission.level])}
        </span>
      ) : (
        <Select
        ...
        >
          <SelectTrigger
            className="h-8 w-28"
            aria-label={t("permissions.permissionLevelFor", { user: permission.user_id })}
          >
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="1">{t("permissions.read")}</SelectItem>
            <SelectItem value="2">{t("permissions.edit")}</SelectItem>
          </SelectContent>
        </Select>
      )}
      {isOwner ? (
        ...
        >
          ...
          <span className="sr-only">{t("permissions.revoke")}</span>
        </Button>
      )}
    </div>
  );
