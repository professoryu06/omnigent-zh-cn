...
describe("IntelligentModelControl — zh-CN", () => {
  afterEach(() => {
    setLocale("en");
  });

  it("renders the aria-label in Chinese", () => {
    setLocale("zh-CN");
    renderControl({ value: "on" });
    expect(trigger()).toHaveAttribute("aria-label", "智能模型路由器");
  });

  it("renders the tooltip title in Chinese", () => {
    setLocale("zh-CN");
    renderControl({ value: "off" });
    openTooltip();
    const titles = screen.getAllByTestId("imc-tooltip-title");
    expect(titles.length).toBeGreaterThan(0);
    for (const title of titles) {
      expect(title.textContent).toBe("智能模型路由器");
    }
  });

  it("renders '已选择 haiku · cheap' in Chinese when verdict is applied", () => {
    setLocale("zh-CN");
    renderControl({ value: "on", verdict: APPLIED_VERDICT });
    openTooltip();
    expect(screen.getAllByTestId("imc-verdict-line")[0].textContent).toBe(
  ...
  it("renders '将会选择 haiku · cheap' in Chinese when verdict is advisory", () => {
    setLocale("zh-CN");
    renderControl({
      value: "on",
      verdict: { ...APPLIED_VERDICT, applied: false },
    });
    openTooltip();
    expect(screen.getAllByTestId("imc-verdict-line")[0].textContent).toBe(
      "将会选择 haiku · cheap",
