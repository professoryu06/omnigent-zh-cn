...
import { I18nProvider, setLocale, translate, useTranslation } from "@/i18n";
...
describe("PermissionsModal", () => {
  ...
  describe("zh-CN i18n", () => {
    it('translates the heading "Share this session" to "共享此会话"', () => {
      expect(translate("permissions.shareThisSession", {}, "zh-CN")).toBe("共享此会话");
    });

    it('translates "Public access" label to "公开访问"', () => {
      expect(translate("permissions.publicAccess", {}, "zh-CN")).toBe("公开访问");
    });

    it('translates "Grant" action to "授权"', () => {
      expect(translate("permissions.grant", {}, "zh-CN")).toBe("授权");
    });

    it('translates "Revoke" action to "撤销"', () => {
      expect(translate("permissions.revoke", {}, "zh-CN")).toBe("撤销");
    });

    it("renders inside I18nProvider with zh-CN locale and exposes translated context", () => {
      setLocale("zh-CN");
      listMock.mockResolvedValue([]);
      ...
      function Reader() {
        ...
        return <span data-testid="translated">{t("permissions.shareThisSession")}</span>;
      }
      ...
      render(
        <>
          <Reader />
          <PermissionsModal sessionId="conv_abc" open={true} onOpenChange={() => {}} />
        </>,
        { wrapper: Wrapper },
      );

      expect(contextLocale).toBe("zh-CN");
      expect(screen.getByTestId("translated")).toHaveTextContent("共享此会话");
      setLocale("en");
    });
  });
});
