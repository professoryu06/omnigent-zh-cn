...
describe("ForkSessionDialog zh-CN", () => {
  afterEach(() => setLocale("en"));
  it("renders the dialog heading in Chinese", () => {
    setLocale("zh-CN");
    renderDialog({});
    expect(screen.getByRole("heading", { name: /克隆会话/ })).toBeInTheDocument();
  });

  it("renders the Host label in Chinese", () => {
    setLocale("zh-CN");
    renderDialog({ sourceWorkspace: "/home/user/project" });
    // The Host label text is rendered in Chinese
    expect(screen.getByText("主机")).toBeInTheDocument();
  });

  it("renders the Host label in English when locale is English", () => {
    setLocale("en");
    renderDialog({ sourceWorkspace: "/home/user/project" });
