...
export function settingsNavGroups(
  ...
  const general: SettingsNavItem[] = [
    { id: "appearance", label: t("settingsNav.appearance"), icon: PaletteIcon },
    { id: "git", label: t("settingsNav.git"), icon: GitBranchIcon },
    { id: "shortcuts", label: t("settingsNav.keyboardShortcuts"), icon: KeyboardIcon, hideOnMobile: true },
  ];
  if (hasAuthSession) {
    general.unshift({ id: "account", label: t("settingsNav.account"), icon: UserCogIcon });
  }
  const groups: SettingsNavGroup[] = [];
  if (isDesktop) {
    groups.push({
      title: t("settingsNav.desktop"),
      items: [{ id: "cli", label: t("settingsNav.localCli"), icon: TerminalIcon }],
    });
  }
  groups.push({ title: t("settingsNav.general"), items: general });
  if (isAdmin) {
    groups.push({
      title: t("settingsNav.admin"),
      items: [
        { id: "members", label: t("settingsNav.members"), icon: UsersIcon },
        { id: "policies", label: t("settingsNav.policies"), icon: ShieldCheckIcon },
        { id: "sharing", label: t("settingsNav.sharing"), icon: Share2Icon },
      ],
    });
  }
  groups.push({
    title: t("settingsNav.archived"),
    items: [{ id: "archived", label: t("settingsNav.archivedSessions"), icon: ArchiveIcon }],
  });
  return groups;
}
...
export function SettingsSidebarBody({
  ...
  const groups = settingsNavGroups(t, hasAuthSession, isElectronShell(), isAdmin);
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              variant="ghost"
              size="icon"
              aria-label={t("settingsNav.closeSidebar")}
              onClick={onClose}
          ...
          <TooltipContent side="bottom">{t("settingsNav.collapseSidebar")}</TooltipContent>
        </Tooltip>
      </div>
