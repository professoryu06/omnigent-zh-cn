...
export function FlatFileList({
  ...
  if (isError) {
    ...
    if (error instanceof RunnerOfflineError) {
      ...
      return <p className="px-2 py-1 text-muted-foreground text-xs">{t("file.noWorkspaceChanges")}</p>;
    }
    return (
  ...
  if (sorted.length === 0) {
    return (
      <p className="px-2 py-1 text-muted-foreground text-xs">
        {t("file.noChangedFilesMatch", { query: searchQuery.trim() })}
      </p>
    );
  }
  return (
    <>
      {hiddenCount > 0 && (
        <p className="px-2 py-1 text-muted-foreground text-xs">
          {hiddenCount === 1
            ? t("file.filesHidden_one", { count: hiddenCount })
            : t("file.filesHidden_other", { count: hiddenCount })}
          <button
            type="button"
            className="cursor-pointer underline hover:text-foreground"
            onClick={onShowHidden}
