...
describe("async delete session flow", () => {
  ...
  it("shows a retryable error row when the delete fails; Retry replays the same args", () => {
    ...
    // The failure copy and session label are one localized, visible sentence
    // so the user can tell which row failed when several deletes fail.
    expect(within(failed).getByText("Couldn't delete My Session")).toBeInTheDocument();
