  ...
  return (
    <div className="flex shrink-0 items-center justify-end gap-0.5 border-b px-2 py-1">
      <Button
        ...
        aria-label={t("subagent.list")}
        title={t("subagent.list")}
      ...
      <Button
        ...
        aria-label={t("subagent.graph")}
        title={t("subagent.graph")}
...
function StatusIndicator({ activity, label, details }: AgentStatus) {
  ...
  const displayLabel =
    activity === "launching"
      ? t("subagent.launching")
      : activity === "working"
        ? t("subagent.working")
        : activity === "awaiting"
          ? t("subagent.needsResponse")
          : activity === "failed"
            ? t("subagent.failed")
            : activity === "disconnected"
              ? t("subagent.disconnected")
              : activity === "done"
                ? t("subagent.done")
                : activity === "idle"
                  ? t("subagent.idle")
                  : label; // "other" — raw server value
  ...
  return (
    ...
    >
      {!QUIET_STATE[activity] && <span>{displayLabel}</span>}
      {activity === "working" ? (
        <RunningDot />
      ) : (
