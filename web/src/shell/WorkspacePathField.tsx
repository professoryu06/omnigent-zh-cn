    ...
    <div ref={containerRef} className="relative">
      <div className="flex items-center gap-2">
        <input
          ...
          autoCorrect="off"
          role="combobox"
          aria-label={t("workspace.workingDirectoryPath")}
        ...
        <button
          type="button"
          onClick={onBrowse}
          aria-label={t("workspace.browseDirectories")}
          ...
          {matches.length > 0 && (
            <>
              ...
              {hiddenMatchCount > 0 && (
                ...
                >
                  {t("workspace.moreMatches", { count: hiddenMatchCount })}
                </div>
              )}
            </>
          )}
          {showLoading && <div className="px-3 py-2 text-xs text-muted-foreground">{t("workspace.loading")}</div>}
        </div>
      )}
    </div>
  );
