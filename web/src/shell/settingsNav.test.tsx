...
// Bound t() for direct settingsNavGroups(t,) calls in tests.
...
describe("settingsNavGroups", () => {
  it("flags Keyboard shortcuts as hidden on mobile, but not the other items", () => {
    const items = settingsNavGroups(t,false, false).flatMap((g) => g.items);
  ...
  it("includes Account (leading) whenever a login session exists (accounts OR OIDC)", () => {
    ...
    expect(
      settingsNavGroups(t,false, false)
        .flatMap((g) => g.items)
        .map((i) => i.id),
    ).not.toContain("account");
    // Any login session → Account appears and leads its group.
    const withAccount = settingsNavGroups(t,true, false)
      .flatMap((g) => g.items)
      .map((i) => i.id);
  ...
  it("includes the Local CLI section only in the desktop shell", () => {
    const ids = (isDesktop: boolean) =>
      settingsNavGroups(t,false, isDesktop)
        .flatMap((g) => g.items)
        .map((i) => i.id);
  ...
  it("includes the Admin group (Members / Policies / Sharing) for any admin, in accounts OR OIDC mode", () => {
    const ids = (accountsEnabled: boolean, isAdmin: boolean) =>
      settingsNavGroups(t,accountsEnabled, false, isAdmin)
        .flatMap((g) => g.items)
        .map((i) => i.id);
    ...
    const accountsAdmin = settingsNavGroups(t,true, false, true).find((g) => g.title === "Admin");
    ...
    const oidcAdmin = settingsNavGroups(t,false, false, true).find((g) => g.title === "Admin");
