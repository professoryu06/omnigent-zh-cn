...
export function WorkspacePicker({
  ...
  async function commitNewFolder() {
    ...
    try {
      ...
      setCreateError(err instanceof Error ? err.message : t("workspace.failedToCreateFolder"));
    }
  }
    ...
    >
      <div className="flex shrink-0 items-center gap-1.5 border-b px-2 py-1.5">
        <button
          ...
          aria-label={t("workspace.upOneLevel")}
          title={t("workspace.upOneLevel")}
        ...
        {onSelect && (
          <Button
            ...
            title={t("workspace.selectThisFolder", { name: basename(currentAbsolute) })}
            className="shrink-0"
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              placeholder={t("workspace.newFolderName")}
              spellCheck={false}
              autoCapitalize="off"
              autoCorrect="off"
            ...
            <button
              ...
              aria-label={t("workspace.cancelNewFolder")}
      ...
      {occupiedCount > 0 && (
        ...
        >
          ...
          <span>
            {occupiedCount === 1
              ? t("workspace.agentWorking_one")
              : t("workspace.agentWorking_other", { count: occupiedCount })}{" "}
            {t("workspace.conflictWarning")}
          </span>
        </div>
      )}
      <div className="min-h-0 flex-1 overflow-y-auto">
        {isLoading && <div className="px-3 py-3 text-xs text-muted-foreground">{t("workspace.loading")}</div>}
        {error !== null && error !== undefined && !isLoading && (
          <div className="px-3 py-3 text-xs text-destructive" data-testid="workspace-picker-error">
            {error instanceof Error ? error.message : t("workspace.failedToLoadDirectory")}
          </div>
        )}
        {!isLoading && error === null && entries.length === 0 && (
          <div className="px-3 py-3 text-xs text-muted-foreground">
            {activeFilter !== null ? t("workspace.noMatchingEntries") : t("workspace.emptyDirectory")}
          </div>
        )}
        {entries.map((entry) => {
