...
describe("getConversationAgentType", () => {
  ...
  it("returns 'Other' (Chinese) when locale is zh-CN", () => {
    setLocale("zh-CN");
    const conv = conversation("conv_plain", "Some chat", new Date(2026, 4, 14, 9));
    expect(getConversationAgentType(conv)).toBe("其他");
    setLocale("en");
  });
  ...
  it("returns 'Other' (Chinese) when agent_name is null and locale is zh-CN", () => {
    setLocale("zh-CN");
    const conv: Conversation = {
      ...conversation("conv_null_name_zh", "Chat", new Date(2026, 4, 14, 9)),
      agent_name: null,
    };
    expect(getConversationAgentType(conv)).toBe("其他");
    setLocale("en");
...
describe("untitledConversationLabel", () => {
  it("returns canonical fallback label in English", () => {
    expect(untitledConversationLabel()).toBe("New session");
  });

  it("returns Chinese fallback label when locale is zh-CN", () => {
    setLocale("zh-CN");
    expect(untitledConversationLabel()).toBe("新建会话");
    setLocale("en");
  });
});
...
describe("conversationDisplayLabel", () => {
  ...
  it("returns Chinese fallback '新建会话' when locale is zh-CN and no title", () => {
    setLocale("zh-CN");
    expect(
      conversationDisplayLabel(
        conversation("conv_abcdefghijklmnopqrstuvwxyz", null, new Date(2026, 4, 14, 9)),
