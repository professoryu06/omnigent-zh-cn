...
export function FolderTree({
  ...
  if (searchQuery.trim().length > 0) {
    if (isSearching && !searchResults) {
      return <p className="px-2 py-1 text-muted-foreground text-xs">{t("file.searching")}</p>;
    }
    if (isSearchError) {
      return (
        <p className="px-2 py-1 text-destructive text-xs">
          {t("file.searchFailed", { error: searchError instanceof Error ? searchError.message : t("status.unknownError") })}
        </p>
      );
    }
    if (!searchResults || searchResults.length === 0) {
      return (
        <p className="px-2 py-1 text-muted-foreground text-xs">
          {t("file.noFilesMatch", { query: searchQuery.trim() })}
        </p>
      );
    }
    ...
    if (visibleResults.length === 0) {
      ...
      return (
        <p className="px-2 py-1 text-muted-foreground text-xs">
          {hiddenCount === 1
            ? t("file.matchesInHiddenDirs_one", { count: hiddenCount })
            : t("file.matchesInHiddenDirs_other", { count: hiddenCount })}{" "}
          <button
            type="button"
  ...
  if (isLoading) {
    return <p className="px-2 py-1 text-muted-foreground text-xs">{t("file.loading")}</p>;
  }
  if (isError) {
    ...
    if (error instanceof RunnerOfflineError) {
      ...
      return <p className="px-2 py-1 text-muted-foreground text-xs">{t("file.noFilesInWorkspace")}</p>;
    }
    return (
      <p className="px-2 py-1 text-destructive text-xs">
        {t("file.loadFailed", { error: error instanceof Error ? error.message : String(error) })}
      </p>
    );
  }
