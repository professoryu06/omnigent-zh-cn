                      ...
                      {inboxCount > 0 && (
                        <span
                          aria-label={
                            inboxCount === 1
                              ? t("sidebar.inboxItem")
                              : t("sidebar.inboxItems", { count: inboxCount })
                          }
              ...
              >
                <TabsList className="w-full">
                  <TabsTrigger value="mine" data-testid="sidebar-tab-mine" className="min-w-0">
                    <span className="min-w-0 truncate">{t("sidebar.mySessions")}</span>
                  </TabsTrigger>
                  <TabsTrigger value="shared" data-testid="sidebar-tab-shared" className="min-w-0">
                    <span className="min-w-0 truncate">{t("sidebar.sharedWithMe")}</span>
                  </TabsTrigger>
                </TabsList>
              </Tabs>
            </div>
          ...
          <div className="md:shrink-0 md:px-3 md:pb-3 max-md:absolute max-md:bottom-3 max-md:left-3 max-md:z-10">
            ...
            >
              ...
              <Link to="/settings" aria-label={t("sidebar.settings")}>
                ...
                <span className="max-md:hidden">{t("sidebar.settings")}</span>
              </Link>
            </Button>
          </div>
        </>
  ...
  return (
    ...
    >
      <ConversationSection
        ...
        footer={
          loadingFirstPage ? (
            <p className="px-2 py-1 pl-7 text-muted-foreground text-xs">{t("sidebar.loading")}</p>
          ) : (
...
function ConversationList({
  ...
  if (conversationsQuery.isLoading) {
    return <p className="px-2 py-1 text-muted-foreground text-xs">{t("sidebar.loading")}</p>;
  }
  if (conversationsQuery.isError) {
    const err = conversationsQuery.error;
    return (
      <p className="px-2 py-1 text-destructive text-xs">
        {t("sidebar.failedToLoad", { error: err instanceof Error ? err.message : String(err) })}
      </p>
    );
  }
    ...
    >
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            ...
                            variant="ghost"
                            size="icon-sm"
                            aria-label={t("sidebar.collapseToPrevious")}
                        ...
                        <TooltipContent side="bottom">{t("sidebar.collapseToPrevious")}</TooltipContent>
                      </Tooltip>
                    );
                  }
                  return (
                    <Tooltip>
                      ...
                      <TooltipContent side="bottom">{t("sidebar.expandAll")}</TooltipContent>
                    </Tooltip>
                  );
                })()}
              >
      ...
      >
        <DialogContent onClick={(e) => e.stopPropagation()}>
          <DialogHeader>
            <DialogTitle>{t("sidebar.removeFromProjectQ")}</DialogTitle>
            <DialogDescription>
              {t("sidebar.removeFromProjectSoleDesc", { project: pendingUngroup?.project ?? "" })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
...
function UngroupDropZone() {
  ...
  return (
    ...
    >
      ...
      {t("sidebar.dropToRemoveFromProject")}
    </div>
  );
}
    ...
    <>
      ...
      {canManage && !sharingOff ? (
        ...
        <Tooltip>
          ...
          <TooltipContent side="left">
            {sharingOff
              ? t("sidebar.shareDisabledServer")
              : t("sidebar.shareDisabledPermission")}
          </TooltipContent>
        </Tooltip>
      )}
      {canEdit ? (
        ...
        <Tooltip>
          ...
          <TooltipContent side="left">
            {t("sidebar.renameDisabledPermission")}
          </TooltipContent>
        </Tooltip>
      )}
      ...
      {canEdit && isOwner && (
        <C.Sub>
          <C.SubTrigger data-testid="move-to-project" className="whitespace-nowrap">
            ...
            {currentProject ? t("sidebar.moveSession") : t("sidebar.addToProject")}
          </C.SubTrigger>
      ...
      {isOwner ? (
        <C.Item data-testid="archive-conversation" onSelect={runArchive}>
          ...
          {isArchived ? t("sidebar.unarchive") : t("sidebar.archive")}
        </C.Item>
      ) : (
        <Tooltip>
          <TooltipTrigger asChild>
          ...
          <TooltipContent side="left">
            {t("sidebar.archiveDisabled", { action: isArchived ? t("sidebar.unarchive") : t("sidebar.archive") })}
          </TooltipContent>
        </Tooltip>
      )}
      {isOwner ? (
    ...
    >
      ...
      <div className="flex w-full items-center gap-1.5">
        <span className="relative min-w-0 truncate">
          {label}
          {hasUnseenMessages && <span className="sr-only">{t("sidebar.unread")}</span>}
        </span>
      </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <DialogHeader>
            <DialogTitle>{t("sidebar.deleteConversationQ")}</DialogTitle>
            <DialogDescription>
              {t("sidebar.deleteConversationDesc", { label })}
            </DialogDescription>
          </DialogHeader>
          {gitBranch !== null && (
            <div className="flex flex-col gap-2 rounded-md border border-destructive/40 bg-destructive/5 p-3">
              <p className="text-xs text-muted-foreground">
                {t("sidebar.deleteCleanupHint")}{" "}
                <span className="font-semibold text-destructive">{t("sidebar.irreversible")}</span>.
              </p>
      ...
      <Dialog open={stopOpen} onOpenChange={setStopOpen}>
        ...
        >
          <DialogHeader>
            <DialogTitle>{t("sidebar.stopSessionQ")}</DialogTitle>
            <DialogDescription>
              {t("sidebar.stopSessionDesc", { label })}
            </DialogDescription>
          </DialogHeader>
          {stopSession.isError && (
            <p className="text-sm text-destructive" role="alert">
              {t("sidebar.couldntStopSession")}
              {stopSession.error instanceof Error && stopSession.error.message
                ? `: ${stopSession.error.message}`
                : t("sidebar.stillRunning")}
              {t("sidebar.tryAgain")}
            </p>
          )}
          <DialogFooter>
      ...
      <Dialog open={removeProjectOpen} onOpenChange={setRemoveProjectOpen}>
        <DialogContent onClick={(e) => e.stopPropagation()}>
          <DialogHeader>
            <DialogTitle>{t("sidebar.removeFromProjectQ")}</DialogTitle>
            <DialogDescription>
              {t("sidebar.removeFromProjectSoleDesc", { project: currentProject })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
...
function DeletingRow({
    ...
    return (
      ...
      >
        ...
        <span
          ...
          title={t("sidebar.couldntDelete", { label })}
        >
          {t("sidebar.couldntDelete", { label })}
        </span>
        ...
        <Button
          ...
          variant="ghost"
          size="icon-sm"
          aria-label={t("sidebar.dismissDeleteError")}
          onClick={onDismiss}
        >
  ...
  return (
    ...
    >
      ...
      <span className="shrink-0 text-xs">{t("sidebar.deleting")}</span>
    </div>
  );
}
...
function ArchivingRow({ label }: { label: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="shrink-0 text-xs">{t("sidebar.archiving")}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center">
      ...
      <Button
        ...
        variant="ghost"
        size="icon-sm"
        aria-label={t("sidebar.newSessionInProject", { projectName })}
  ...
  return (
    <>
      <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            variant="ghost"
            size="icon-sm"
            aria-label={t("sidebar.projectActionsFor", { projectName })}
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent onClick={(e) => e.stopPropagation()}>
          <DialogHeader>
            <DialogTitle>{t("sidebar.deleteProjectQ")}</DialogTitle>
            <DialogDescription>
          ...
          {deleteProject.isError && (
            <p className="text-sm text-destructive" role="alert">
              {t("sidebar.deleteProjectPartialError")}
            </p>
          )}
          <DialogFooter className="border-t-0 bg-transparent">
            <Button
            ...
            >
              {deleteProject.isPending ? t("sidebar.deleting") : t("sidebar.deleteProject")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    ...
    <>
      ...
      <div className="max-h-48 overflow-y-auto">
        ...
        {filtered.length === 0 && !creatingNew && (
          <p className="px-2 py-1.5 text-xs text-muted-foreground">{t("sidebar.noProjectsYet")}</p>
        )}
      </div>
      <div className="border-t pt-1">
        ...
        {currentProject && (
          <C.Item className="px-2 py-1" onSelect={() => onSelect("")}>
            {t("sidebar.removeFromProjectName", { currentProject })}
          </C.Item>
        )}
      </div>
    </>
  ...
  return (
    <>
      <div className="relative mt-3 flex flex-col gap-1.5">
        <div className="relative flex min-h-8 items-center gap-1.5 px-2 pr-9">
          <span className="shrink-0 whitespace-nowrap text-sm text-muted-foreground">
            {count === 0 ? t("sidebar.noneSelected") : t("sidebar.countSelected", { count })}
          </span>
          <Button
            type="button"
            variant="ghost"
          ...
          <Tooltip>
            ...
            <TooltipContent side="bottom">{t("sidebar.exitSelection")}</TooltipContent>
          </Tooltip>
        </div>
        ...
        <div className="flex items-center gap-1.5 px-2">
          ...
          >
            ...
            {t("sidebar.bulkDeleteCount", { count: ownedSelected.length })}
          </Button>
        </div>

        {(bulkArchive.isError || bulkDelete.isError) && (
          <p className="text-xs text-destructive" role="alert">
            {t("sidebar.bulkActionsFailed")}
          </p>
        )}
      </div>
      ...
      <Dialog open={confirmDeleteOpen} onOpenChange={setConfirmDeleteOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("sidebar.bulkDeleteQ", { count: ownedSelected.length })}</DialogTitle>
            <DialogDescription>
              {t("sidebar.bulkDeleteDesc")}
            </DialogDescription>
          </DialogHeader>
          <p className="flex items-start gap-2 rounded-md border border-warning/40 bg-warning/5 p-3 text-xs text-muted-foreground">
            ...
            {t("sidebar.bulkDeleteBranchWarning")}
          </p>
          <DialogFooter className="border-t-0 bg-transparent">
            <Button
              type="button"
            ...
            <Button
              ...
              disabled={bulkDelete.isPending}
            >
              {t("sidebar.bulkDeleteConfirm", { count: ownedSelected.length })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
