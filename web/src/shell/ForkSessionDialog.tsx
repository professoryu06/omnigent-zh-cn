...
export function ForkSessionForm({
  ...
  async function handleFork(): Promise<void> {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("session.cloneFailed"));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  const namePlaceholder = defaultForkTitle(sourceTitle) || t("session.nameClonedSession");
      ...
      <div className="-mr-4 flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto pr-4 [scrollbar-width:thin] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar-track]:bg-transparent">
        ...
        {isCodingSource && (
          <div className="flex flex-col gap-2">
            <span className="text-xs font-medium text-muted-foreground">{t("session.host")}</span>
            {hosts === undefined ? (
              <p className="text-xs text-muted-foreground" data-testid="fork-session-no-hosts">
                {t("common.loadingHosts")}
              </p>
              ...
              <ConnectHostInstructions
                ...
                label={
                  allHosts.length === 0
                    ? t("session.noHostsConnected")
                    : t("session.noHostsOnline")
                }
              />
            ) : (
              <>
                ...
                >
                  <SelectTrigger className="w-full text-xs" data-testid="fork-session-host-select">
                    <SelectValue placeholder={t("common.selectHost")} />
                  </SelectTrigger>
                  <SelectContent>
            ...
            >
              ...
              <SelectValue>
                {switching ? (
                  ...
                  <>
                    {sourceAgentDisplay}{" "}
                    <span className="text-muted-foreground">{t("session.sameAsOriginal")}</span>
                  </>
                )}
              </SelectValue>
            </SelectTrigger>
        ...
        {usingSourceDir && (
          <p className="text-xs text-muted-foreground" data-testid="fork-session-reuse-dir-hint">
            {t("session.cloneReusesDirPrefix")}{" "}
            <Tooltip>
            ...
            {t("session.cloneReusesDirSuffix")}
          </p>
        )}
          ...
          >
            ...
            <span>
              {conflictingSessions.length === 1
                ? t("session.directoryConflictHintSingular")
                : t("session.directoryConflictHintPlural", { count: conflictingSessions.length })}
            </span>
          </p>
        )}
                      ...
                      <>
                        ...
                        {showMismatchWarning && (
                          ...
                          >
                            <AlertTriangleIcon className="mt-0.5 size-3.5 shrink-0" />
                            <span>
                              {t("session.directoryMismatchWarning")}
                            </span>
                          </p>
                        )}
                      </>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {branchName.trim() !== "" && (
                      <input
                        ...
                        placeholder={t("session.baseBranchPlaceholder")}
...
export function ForkSessionDialog({
  ...
  const cloneDescription = t(
    truncated
      ? sourceWorkspace
        ? "session.cloneTruncatedCodingDescription"
        : "session.cloneTruncatedDescription"
      : sourceWorkspace
        ? "session.cloneFullCodingDescription"
        : "session.cloneFullDescription",
  );
      ...
      >
        <DialogHeader>
          <DialogTitle className="flex items-center gap-1.5">
            {truncated ? t("session.forkFromResponse") : t("session.cloneSession")}
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  aria-label={t("session.whatIsCloning")}
