...
export function AddAgentDialog({
  ...
  async function handleAdd(): Promise<void> {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("agent.couldntAdd"));
    } finally {
      setSubmitting(false);
    }
  }
      ...
      >
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto">
          <div className="flex flex-col gap-2">
            <span className="text-xs font-medium text-muted-foreground">{t("agent.pickAnAgent")}</span>
            {agentList.length === 0 ? (
              <p data-testid="add-agent-empty" className="text-xs text-muted-foreground">
                {t("agent.noAgentsAvailable", { command: "omnigent server --agent" })}
              </p>
            ) : (
              agentList.map((agent) => (
                <AgentCard
        ...
        <DialogFooter>
          ...
          >
            {submitting ? t("agent.adding") : t("agent.add")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
