...
function HiddenFilesToggle({
  ...
  const ariaLabel = showHidden ? t("file.hideHiddenFiles") : t("file.showHiddenFiles");
  const tooltipLabel = showHidden
    ? t("file.hideHiddenFiles")
    : hasHidden
      ? hiddenCount === 1
        ? t("file.hiddenFilesTooltip_one", { count: hiddenCount })
        : t("file.hiddenFilesTooltip_other", { count: hiddenCount })
      : t("file.showHiddenFiles");
...
const SORT_OPTIONS: { value: ChangedSort; labelKey: string; Icon: typeof ArrowDownAZIcon }[] = [
  { value: "alpha", labelKey: "file.sortFilename", Icon: ArrowDownAZIcon },
  { value: "recent", labelKey: "file.sortLastEdited", Icon: FileClockIcon },
  { value: "size", labelKey: "file.sortSize", Icon: ArrowDownWideNarrowIcon },
  { value: "type", labelKey: "file.sortType", Icon: FileTypeIcon },
];
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          aria-label={t("file.sortActive", { value: t(active.labelKey) })}
...
function FileScopeSwitch({
  ...
  return (
    <div role="radiogroup" aria-label={t("file.fileScope")} className="flex shrink-0 items-center gap-1">
      <button
        type="button"
        role="radio"
        aria-checked={changedSelected}
        aria-label={t("file.changed")}
        title={t("file.showChangedOnly")}
      ...
      <button
        ...
        aria-label={t("file.all")}
        title={t("file.showFullFolderTree")}
  ...
  return (
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-2 px-3 py-2">
        <span className="shrink-0 font-medium text-sm">{t("file.workingFolder")}</span>
        ...
        >
          ...
          <div className="flex min-w-0 flex-1 items-center gap-2">
            <div className="flex min-w-0 flex-1 items-center gap-[6px] rounded-full border border-border px-[10px] py-[4px] transition-colors focus-within:border-border-strong">
              ...
              <input
                aria-label={t("file.searchChangedFiles")}
        ...
        <div className="shrink-0" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center gap-2 px-2 py-1.5 @max-[400px]/filespanel:flex-col @max-[400px]/filespanel:items-stretch">
            ...
            <div className="flex min-w-0 flex-1 items-center gap-2">
              <div className="flex min-w-0 flex-1 items-center gap-[6px] rounded-full border border-border px-[10px] py-[4px] transition-colors focus-within:border-border-strong">
                <SearchIcon className="size-4 shrink-0 text-muted-foreground" />
                <input
                  aria-label={t("file.searchAllFiles")}
                  className="min-w-0 flex-1 bg-transparent text-xs outline-none placeholder:text-muted-foreground"
              ...
              <button
                type="button"
                aria-label={showSearchFilters ? t("file.hideSearchFilters") : t("file.showSearchFilters")}
                aria-expanded={showSearchFilters}
                title={t("file.filesToIncludeExclude")}
                className={cn(
          ...
          {showSearchFilters && (
            <div className="flex flex-col gap-1.5 border-border border-t px-3 py-2">
              <SearchFilterInput
                label={t("file.filesToInclude")}
                placeholder={t("file.includePatternsPlaceholder")}
                value={treeInclude}
                onChange={setTreeInclude}
              />
              <SearchFilterInput
                label={t("file.filesToExclude")}
                placeholder={t("file.excludePatternsPlaceholder")}
                value={treeExclude}
