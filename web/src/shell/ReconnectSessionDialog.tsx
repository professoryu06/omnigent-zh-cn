...
export function ReconnectSessionDialog({
  ...
  const title = isHostReconnect ? t("session.hostOffline") : t("session.agentDisconnected");
  const description = isHostReconnect
    ? isOwner
      ? t("session.hostOwnerDescription")
      : t("session.hostViewerDescription")
    : t("session.runDescription");
  return (
