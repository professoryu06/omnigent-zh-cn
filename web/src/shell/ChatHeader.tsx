      ...
      <div className={cn("flex items-center gap-1", !sidebarOpen && "traffic-light-clearance")}>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                variant="ghost"
                size="icon"
                aria-label={t("chatHeader.openSidebar")}
            ...
            <TooltipContent side="bottom">{t("chatHeader.openSidebar")}</TooltipContent>
          </Tooltip>
        )}
        {isChildSession && parentSessionId && (
          <>
            ...
            >
              <Link to={`/c/${parentSessionId}`} aria-label={t("chatHeader.backToParentSession")}>
                <ChevronLeftIcon className="size-4" />
                <span>{t("chatHeader.back")}</span>
              </Link>
            </Button>
            ...
            <div className="flex min-w-0 items-center gap-2">
              ...
              {boundAgent?.name ? (
                <div className="flex min-w-0 flex-col leading-tight">
                  ...
                  <span className="text-xs text-muted-foreground">{t("chatHeader.subAgent")}</span>
                </div>
              ) : (
                <span className="text-sm font-semibold text-foreground">{t("chatHeader.subAgent")}</span>
              )}
            </div>
          </>
        )}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                variant="ghost"
                size="icon"
                aria-label={t("chatHeader.sessionActions")}
        ...
        {canShare && shareDisabled && shareDisabledReason ? (
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              <span
                tabIndex={0}
                aria-label={t("chatHeader.shareSessionDisabled", { reason: shareDisabledReason })}
                className="hidden md:inline-flex"
              >
                <Button
                  type="button"
                  aria-label={t("chatHeader.shareSession")}
                  disabled
                  title={shareDisabledReason}
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                variant="ghost"
                size="icon"
                aria-label={rightPanelOpen ? t("chatHeader.collapseRightPanel") : t("chatHeader.expandRightPanel")}
            ...
            <TooltipContent>
              {rightPanelOpen ? t("chatHeader.collapseRightPanel") : t("chatHeader.expandRightPanel")}
            </TooltipContent>
          </Tooltip>
        )}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  variant="ghost"
                  size="icon"
                  aria-label={t("chatHeader.openSessionMenu")}
                  className="text-muted-foreground hover:text-foreground md:hidden"
                >
                  <PanelRightIcon className="size-4" />
                </Button>
