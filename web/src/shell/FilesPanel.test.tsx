...
describe("FilesPanel sort control", () => {
  ...
  it("updates the active sort label when the locale is Chinese", () => {
    setLocale("zh-CN");
    renderPanel({
      conversationId: "conv_chinese_sort",
      files: [file("a.txt")],
      flatView: false,
    });

    expect(screen.getByRole("button", { name: "排序：最近编辑" })).toBeInTheDocument();
    setLocale("en");
  });
});

describe("FilesPanel include/exclude placeholder i18n", () => {
  it("shows English placeholders for the include/exclude inputs", () => {
    renderPanel({
      conversationId: "conv_placeholders_en",
      files: [file("src/App.tsx")],
      treeSearchResults: [],
    });
    ...
    const includeInput = screen.getByRole("textbox", { name: "files to include" });
    const excludeInput = screen.getByRole("textbox", { name: "files to exclude" });

    expect(includeInput).toHaveAttribute("placeholder", "e.g. *.ts, src/**");
    expect(excludeInput).toHaveAttribute(
      "placeholder",
      "e.g. **/node_modules, *.test.ts",
    );
  });

  it("shows Chinese placeholders for the include/exclude inputs when locale is zh-CN", () => {
    setLocale("zh-CN");
    renderPanel({
      conversationId: "conv_placeholders_zh",
      files: [file("src/App.tsx")],
      treeSearchResults: [],
    });

    fireEvent.click(screen.getByRole("button", { name: "显示搜索筛选器" }));

    const includeInput = screen.getByRole("textbox", { name: "要包含的文件" });
    const excludeInput = screen.getByRole("textbox", { name: "要排除的文件" });

    expect(includeInput).toHaveAttribute("placeholder", "例如：*.ts, src/**");
    expect(excludeInput).toHaveAttribute(
      "placeholder",
