...
describe("Sidebar session list", () => {
  it("renders core navigation in Simplified Chinese when Chinese is selected", () => {
    setLocale("zh-CN");
  ...
  it("renders settings nav with Chinese labels when Chinese is selected", () => {
    setLocale("zh-CN");
    mockConversations(THREE_TYPE_CONVERSATIONS);

    renderSidebar(true, "/settings");

    expect(screen.getByRole("link", { name: /返回 Omnigent/ })).toBeInTheDocument();
    expect(screen.getByTestId("settings-nav-appearance")).toBeInTheDocument();
    expect(screen.getByText("外观")).toBeInTheDocument();
    expect(screen.getByText("Git")).toBeInTheDocument();
    expect(screen.getByTestId("settings-nav-archived")).toBeInTheDocument();
...
describe("Sidebar zh-CN", () => {
  afterEach(() => setLocale("en"));
  it("renders section headers in Chinese", () => {
    setLocale("zh-CN");
    mockConversations(THREE_TYPE_CONVERSATIONS);
    renderSidebar();
...
describe("Sidebar zh-CN project dialog", () => {
  afterEach(() => setLocale("en"));
  it("renders the remove-from-project confirmation dialog in Chinese", async () => {
    setLocale("zh-CN");
    projectsMock.push("Sprint 42");
    // A single session filed under Sprint 42
    mockConversations([
      conv("conv_filed", "Claude Code", { labels: { omni_project: "Sprint 42" } }),
    ]);
    // Server says only this one session is in the project
    fetchProjectSessionIdsMock.mockResolvedValue(["conv_filed"]);
    renderSidebar();

    // Expand the project folder
    fireEvent.click(screen.getByRole("button", { name: "Sprint 42" }));
    // Open kebab menu on the conversation row using the accessible testid trigger
    const row = screen.getByRole("link", { name: /conv_filed/ }).closest("li")!;
    fireEvent.pointerDown(within(row).getByTestId("conversation-actions"), {
      button: 0,
      ctrlKey: false,
    });
    // Open the "Move session" submenu
    fireEvent.click(await screen.findByTestId("move-to-project"));
    // In zh-CN, the remove item reads "从 Sprint 42 中移除"
    const removeItem = await screen.findByRole("menuitem", { name: /从.*Sprint 42.*移除/ });
    fireEvent.click(removeItem);

    // Assert Chinese dialog title
    expect(await screen.findByText("从项目中移除？")).toBeInTheDocument();
    // Assert the destructive action button
    expect(screen.getByRole("button", { name: "从项目中移除" })).toBeInTheDocument();

    // Cancel the dialog, verify no mutation fired
    fireEvent.click(screen.getByRole("button", { name: "取消" }));
    await waitFor(() => expect(screen.queryByText("从项目中移除？")).toBeNull());
    expect(moveToProjectSpy).not.toHaveBeenCalled();
  });
});