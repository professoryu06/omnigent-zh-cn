...
export function SwitchAgentDialog({
  ...
  async function handleSwitch(): Promise<void> {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("agent.couldntSwitch"));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent data-testid="switch-agent-dialog" className="flex flex-col gap-4 sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{t("agent.switchAgent")}</DialogTitle>
          <DialogDescription>
            {t("agent.switchDescription")}
          </DialogDescription>
        </DialogHeader>
                ...
                placeholder={
                  currentDisplay ? (
                    <span data-testid="switch-agent-current">
                      ...
                      <span className="text-muted-foreground">({t("agent.currentAgent")})</span>
                    </span>
                  ) : (
                    t("agent.chooseAgent")
                  )
        ...
        {resetsModelSettings && (
          <p data-testid="switch-agent-reset-warning" className="text-xs text-muted-foreground">
            {t("agent.switchResetsModel", { name: chosen?.display_name ?? "" })}
          </p>
        )}
        ...
        <DialogFooter>
          ...
          >
            {submitting ? t("agent.switching") : t("agent.switchAgent")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
