...
export function ResumeWithDirectoryDialog({
  ...
  async function handleBind(): Promise<void> {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("session.startFailed"));
    } finally {
      setSubmitting(false);
    }
  }
        ...
        {sourceLoading || !hostsLoaded ? (
          ...
          <>
            <div className="flex flex-col gap-2">
              <span className="text-xs font-medium text-muted-foreground">{t("common.host")}</span>
            ...
            <div className="flex flex-col gap-2">
              <span className="text-xs font-medium text-muted-foreground">{t("session.workingDirectory")}</span>
              {selectedHostId ? (
                <>
                  <WorkspacePathField
            ...
            <DialogFooter>
              <Button
                ...
                onClick={handleBind}
              >
                {submitting ? t("common.starting") : t("session.startSession")}
              </Button>
            </DialogFooter>
          </>
        )}
