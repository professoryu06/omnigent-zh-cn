...
export async function describeCreateError(res: Response): Promise<string> {
  ...
  return translate("newSession.createSessionFailed", { status: res.status });
}
...
export function harnessWarningBadgeText(reason: string | null): string {
  if (reason === "binary-missing") return translate("newSession.harnessBinaryMissing");
  if (reason === "needs-auth") return translate("newSession.harnessNeedsAuth");
  return translate("newSession.harnessNeedsSetup");
}
...
export function harnessWarningMessageText(
  ...
  if (reason === "needs-auth") {
    return translate("newSession.harnessNeedsCodexAuth", { agentName: agentName ?? "", hostName: hostName ?? "" });
  }
  if (reason === "binary-missing") {
    return translate("newSession.harnessMissingCodexBinary", { agentName: agentName ?? "", hostName: hostName ?? "" });
  }
  return translate("newSession.harnessNotConfigured", { agentName: agentName ?? "", hostName: hostName ?? "" });
}
...
function harnessWarningMessage(
  ...
  if (reason === "needs-auth") {
    const msg = translate("newSession.harnessNeedsCodexAuth", { agentName: agentName ?? "", hostName: hostName ?? "" });
    const idx = msg.indexOf("codex login");
    return idx >= 0 ? <>{msg.slice(0, idx)}<code>codex login</code>{msg.slice(idx + "codex login".length)}</> : <>{msg}</>;
  }
  if (reason === "binary-missing") {
    const msg = translate("newSession.harnessMissingCodexBinary", { agentName: agentName ?? "", hostName: hostName ?? "" });
    const idx = msg.indexOf("omnigent setup");
    return idx >= 0 ? <>{msg.slice(0, idx)}<code>omnigent setup</code>{msg.slice(idx + "omnigent setup".length)}</> : <>{msg}</>;
  }
  const msg = translate("newSession.harnessNotConfigured", { agentName: agentName ?? "", hostName: hostName ?? "" });
  const idx = msg.indexOf("omnigent setup");
  return idx >= 0 ? <>{msg.slice(0, idx)}<code>omnigent setup</code>{msg.slice(idx + "omnigent setup".length)}</> : <>{msg}</>;
}
      ...
      >
        ...
        <div className="max-h-48 overflow-y-auto">
          <button type="button" className={itemClass} onClick={() => pick("")}>
            <span className="flex-1 truncate">{t("newSession.noProject")}</span>
          ...
          {filtered.length === 0 && !creatingNew && (
            <p className="px-2 py-1.5 text-xs text-muted-foreground">{t("newSession.noProjectsYet")}</p>
          )}
        </div>
    ...
    <div className="px-2 py-1.5">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 text-xs font-medium text-destructive">
          ...
          <span>{t("newSession.bypassApprovalsSandbox")}</span>
        </div>
        <Switch
          size="sm"
          checked={enabled}
          ...
          aria-label={t("newSession.bypassApprovalsSandbox")}
      ...
      {!enabled && (
        <Input
          ...
          aria-label={t("newSession.bypassConfirmationAriaLabel")}
      ...
      {enabled && (
        ...
        >
          ...
          <span>
            {t("newSession.bypassDangerBanner")}
          </span>
        </div>
      )}
    </div>
...
function ModelEffortOptions({
  ...
  return (
    <>
      <div className="px-2 pt-1.5 pb-0.5 text-[11px] font-medium text-muted-foreground">{t("newSession.model")}</div>
      ...
      <DropdownMenuSeparator />
      <div className="px-2 pt-1.5 pb-0.5 text-[11px] font-medium text-muted-foreground">{t("newSession.effort")}</div>
    ...
    if (nativeAgentHasCapability(agent, "permissionMode")) {
      return (
        <>
          ...
          />
          <DropdownMenuSeparator />
          <PickerSectionHeader>{t("newSession.permissionMode")}</PickerSectionHeader>
      ...
      <DropdownMenuTrigger asChild>
        ...
        >
          <span className="max-w-[12rem] truncate text-xs text-foreground">
            {hasAgents ? agentLabel : t("newSession.noAgents")}
          </span>
        ...
        {showMobileKnobs && mobileKnobsAgent ? (
          ...
          <>
            ...
            {harnessEntries.length > 0 && (
              <>
                <PickerSectionHeader>{t("newSession.harnesses")}</PickerSectionHeader>
            ...
            <PickerSectionHeader>{t("newSession.agents")}</PickerSectionHeader>
            ...
            {pendingAgent && (
              ...
              >
                <div className="flex min-w-0 flex-1 items-baseline gap-2.5">
                  ...
                  <span className="truncate text-[11px] text-muted-foreground/70">{t("newSession.custom")}</span>
                </div>
              </DropdownMenuItem>
            )}
            <DropdownMenuItem
            ...
            >
              ...
              {t("newSession.createCustomAgent")}
            </DropdownMenuItem>
          </>
        )}
      </DropdownMenuContent>
...
export function NewChatLandingScreen() {
  ...
  const submitDisabledReason = canSubmit
    ...
    : sandboxSelected && !sandboxRepoValid
      ? t("newSession.invalidRepoUrl")
      : !sandboxSelected && (!selectedHostId || !workspaceValid)
        ? t("newSession.chooseHostAndDir")
        : message.trim().length === 0
          ? t("newSession.enterMessage")
          : null;
  ...
  const hostLabel = connectingThisMachine
    ? t("newSession.connecting")
    : sandboxSelected
      ? sandboxLabel
      : (selectedHost?.name ?? (onlineHosts.length === 0 ? t("newSession.noHosts") : t("newSession.selectHost")));
  ...
  const worktreeLabel = branchName.trim() || t("newSession.noWorktree");
  ...
  // redundant.
  const agentLabel = selectedAgent ? selectedAgent.display_name : t("newSession.selectAgent");
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setCreateError(t("newSession.serverUnreachable"));
    } finally {
      setCreating(false);
    }
  }
    ...
    >
        ...
        <div className="relative flex w-full flex-col gap-3">
          ...
          >
            {isDragActive && (
              <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center rounded-2xl bg-card/80">
                <span className="text-sm font-medium text-ring">{t("newSession.dropFilesHere")}</span>
              </div>
            )}
            ...
            <textarea
              ...
              placeholder={pillSkills.length > 0 ? "" : t("newSession.taskPlaceholder")}
              aria-label={t("newSession.taskPlaceholder")}
              rows={1}
              autoFocus
                ...
                {mentionedItems.map((item, i) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      aria-label={t("newSession.removeFile", { name: item.path })}
                    >
                ...
                {files.map((file, i) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="ml-0.5 rounded-full hover:text-foreground"
                      aria-label={t("newSession.removeFile", { name: file.name || "image.png" })}
                    >
                      <XIcon className="size-3" />
                    </button>
                  </span>
            ...
            <div className="flex items-center justify-between pt-1 pr-4 pb-3 pl-2">
              ...
              <div className="flex items-center gap-0.5">
                <Button
                  ...
                  title={t("newSession.attachFiles")}
                  data-testid="new-chat-landing-attach"
                >
                  <PaperclipIcon className="size-4" />
                  <span className="sr-only">{t("newSession.attachFiles")}</span>
                </Button>
                <ComposerMicButton
                  disabled={creating}
                ...
                <DropdownMenuContent align="start" className="min-w-52">
                    ...
                    <>
                      {managedSandboxesEnabled ? (
                        ...
                        >
                          <span className="flex items-center gap-2">
                            <MonitorCloudIcon className="size-4 text-muted-foreground" />
                            <span className="text-xs">{t("newSession.newSandboxDisabled")}</span>
                          </span>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button
                                ...
                                aria-label={t("newSession.whySandboxUnavailable")}
                  ...
                  {onlineHosts.map((host) => (
                    ...
                    >
                      <HostOption
                        host={host}
                        subtitle={host.host_id === thisMachineHostId ? t("newSession.thisMachine") : undefined}
                        />
                      </DropdownMenuItem>
                    ))}
                          ...
                          <HostOption
                            ...
                            subtitle={
                              connectingThisMachine
                                ? t("newSession.connectingLower")
                                : t("newSession.thisMachineSelectToConnect")
                            }
                          />
                        </DropdownMenuItem>
                      );
                  ...
                  {showConnectThisMachine && (
                    ...
                    >
                      ...
                      <span className="text-xs">
                        {connectingThisMachine ? t("newSession.connectingThisMachine") : t("newSession.runOnThisMachine")}
                      </span>
                    </DropdownMenuItem>
                  )}
                  ...
                  <PopoverContent align="start" className="w-96 p-3">
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        >
                          {t("newSession.repositoryOptional")}
                        </label>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button
                                ...
                                aria-label={t("newSession.databricksGitCredentials")}
                              >
                                <CircleHelpIcon className="size-3.5" />
                              </button>
                            </TooltipTrigger>
                      ...
                      <input
                        ...
                        placeholder={t("newSession.branchDefaultsToReposDefault")}
                        aria-label={t("newSession.repositoryBranch")}
                        className="rounded-md border border-input bg-background px-3 py-2 text-xs outline-none transition-colors focus-visible:border-ring"
                        data-testid="new-chat-landing-repo-branch-input"
                      />
                      <p className="text-xs text-muted-foreground">
                        {t("newSession.sandboxCloneHint")}
                      </p>
                    </div>
                  </PopoverContent>
                </Popover>
                ...
                <Popover open={workspacePopoverOpen} onOpenChange={setWorkspacePopoverOpen}>
                  ...
                  <PopoverContent align="start" className="w-[min(420px,calc(100vw-2rem))] p-0">
                    {selectedHostId ? (
                      ...
                      <p className="p-3 text-xs text-muted-foreground">{t("newSession.selectHostFirst")}</p>
                    )}
                  </PopoverContent>
                </Popover>
              )}
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="relative flex flex-col">
                        ...
                        <button
                          ...
                          title={t("newSession.generateBranchName")}
                          aria-label={t("newSession.generateBranchName")}
                        ...
                        {branchInputFocused && filteredWorktrees.length > 0 && (
                          ...
                          >
                            <span className="px-2 pt-1 pb-0.5 text-[11px] font-medium tracking-wide text-muted-foreground uppercase">
                              {t("newSession.existingWorktrees")}
                            </span>
                                  ...
                                  <li key={w.path}>
                                    ...
                                    >
                                      <span className="font-medium text-foreground">
                                        {w.branch ?? t("newSession.detached")}
                                      </span>
                      ...
                      {branchName.trim() !== "" && !startInExistingWorktree && (
                        <input
                          ...
                          placeholder={t("newSession.baseBranchDefaultsToCurrent")}
                          aria-label={t("session.baseBranchAriaLabel")}
                          className="rounded-md border border-input bg-background px-3 py-2 text-xs outline-none transition-colors focus-visible:border-ring"
                      ...
                      {startInExistingWorktree && (
                        ...
                        >
                          {t("newSession.startsInExistingWorktree")}
                        </p>
                      )}
                    </div>
                  </PopoverContent>
          ...
          {supportsApprovalMode && bypassSandbox && (
            ...
            >
              <TriangleAlertIcon className="size-3.5 shrink-0" />
              <span>
                {t("newSession.codexBypassActiveBanner")}
              </span>
            </p>
          )}
      ...
      <Dialog open={connectOpen} onOpenChange={setConnectOpen}>
        <DialogContent className="sm:max-w-lg" data-testid="connect-host-dialog">
          <DialogHeader>
            <DialogTitle>{t("newSession.connectHost")}</DialogTitle>
          </DialogHeader>
          <ConnectHostInstructions
            serverUrl={serverUrl}
            label={t("newSession.connectHostInstructions")}
          />
        </DialogContent>
      </Dialog>
