...
describe("FileViewer markdown preview/edit/source modes", () => {
  ...
  it("shows the unsaved-changes dialog in Chinese when locale is zh-CN", () => {
    setLocale("zh-CN");
    renderViewer({ open: true, path: "notes.md" });
    // Radix DropdownMenu opens on pointerDown, not click
    fireEvent.pointerDown(screen.getByRole("button", { name: /查看模式：/ }), { button: 0 });
    fireEvent.click(screen.getByRole("menuitem", { name: "编辑" }));
    expect(viewModeOf()).toBe("editor");
    // Mark dirty and trigger discard dialog
    fireEvent.click(screen.getByRole("button", { name: "make dirty" }));
    fireEvent.pointerDown(screen.getByRole("button", { name: /查看模式：/ }), { button: 0 });
    fireEvent.click(screen.getByRole("menuitem", { name: "源码" }));
    // Assert Chinese dialog text
