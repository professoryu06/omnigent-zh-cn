...
describe("FolderTree zh-CN", () => {
  afterEach(() => {
    cleanup();
    setLocale("en");
  });
  it("renders Chinese empty workspace text", () => {
    setLocale("zh-CN");
    renderTree({ files: undefined, isLoading: false, isError: false });
    expect(screen.getByText(/工作区中没有文件/)).toBeInTheDocument();
  });
  it("renders Chinese search error fallback when searchError is a string", () => {
    setLocale("zh-CN");
    renderTree({
      files: [],
      isLoading: false,
      ...
      isSearching: false,
      isSearchError: true,
      searchError: "some string error" as unknown as Error,
    });
    // Non-Error searchError → falls back to t("status.unknownError") = "未知错误"
    // inside t("file.searchFailed", { error: ... }) = "搜索失败：未知错误"
