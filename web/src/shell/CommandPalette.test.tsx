...
describe("CommandPalette — empty state", () => {
  ...
  it("shows Chinese placeholder and actions heading when locale is zh-CN", () => {
    setLocale("zh-CN");
    render(<CommandPalette open onOpenChange={vi.fn()} onToggleLeftSidebar={vi.fn()} onToggleRightSidebar={vi.fn()} />);
    expect(screen.getByPlaceholderText("搜索会话或执行命令")).toBeInTheDocument();
    setLocale("en");
  });

  it("updates action labels when the active locale changes", () => {
    setLocale("en");
    render(
      <I18nProvider>
        <LocaleSwitch />
        <CommandPalette open onOpenChange={vi.fn()} onToggleLeftSidebar={vi.fn()} onToggleRightSidebar={vi.fn()} />
      </I18nProvider>,
    );
