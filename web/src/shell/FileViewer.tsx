...
function FileViewerBody({
  ...
  if (lang === "markdown" && viewMode !== "diff") {
    ...
    const modeOptions: ToolbarOption[] = [
      {
        ...
        tooltip: t("file.renderedPreview"),
      ...
      },
      {
        key: "md-source",
        label: t("file.source"),
        tooltip: t("file.rawMarkdownSource"),
    ...
    toolbarActions.push({
      key: "md-view-mode",
      label: t("file.viewMode", { mode: activeMode.label }),
      tooltip: t("file.viewModeTooltip"),
    ...
    toolbarActions.push({
      key: "preview",
      label: viewMode === "preview" ? t("file.viewSource") : t("file.viewPreview"),
      icon:
  ...
  toolbarActions.push({
    key: "comments",
    label: commentsOpen ? t("file.hideComments") : t("file.showComments"),
  ...
  if (isDiffAvailable) {
    toolbarActions.push({
      key: "diff",
      label: viewMode === "diff" ? t("file.exitDiffView") : t("file.showDiff"),
  ...
  if (viewMode === "diff" && splitToggleAvailable) {
    toolbarActions.push({
      key: "diff-layout",
      label: diffLayout === "unified" ? t("file.splitView") : t("file.unifiedView"),
      icon:
        diffLayout === "unified" ? (
  ...
  if (viewMode === "diff") {
    toolbarActions.push({
      key: "hide-whitespace",
      label: hideWhitespace ? t("file.showWhitespaceChanges") : t("file.hideWhitespaceChanges"),
  ...
  toolbarActions.push({
    key: "copy-link",
    label: t("file.copyLinkToFile"),
    tooltip: linkCopied ? t("file.copied") : t("file.copyLink"),
    icon: linkCopied ? (
    ...
    <>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      variant="ghost"
                      size="icon-sm"
                      aria-label={t("file.closeFileViewer")}
                      onClick={() => guardDirty(onClose)}
                    >
                      <ArrowLeftIcon className="size-4" />
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>{t("file.close")}</TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
          )}
          {showNavButtons && (
            <div ref={toolbarNavRef} className="flex items-center gap-0.5 shrink-0">
              <Button
                ...
                variant="ghost"
                size="icon-sm"
                aria-label={t("file.previousFile")}
                disabled={!prevPath}
        ...
        >
          ...
          <div className="flex items-center justify-end gap-1">
            {toolbarCollapsed ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button type="button" variant="ghost" size="icon-sm" aria-label={t("file.moreActions")}>
          ...
          {isDeletedFile && viewMode !== "diff" ? (
            ...
            diffQuery.isError ? (
              <div className="flex items-center justify-center p-8 text-destructive text-sm">
                {t("file.loadFailed", {
                  error: diffQuery.error instanceof Error
      ...
      >
        <DialogContent showCloseButton={false}>
          <DialogHeader>
            <DialogTitle>{t("file.unsavedChangesTitle")}</DialogTitle>
            <DialogDescription>
              {t("file.unsavedChangesDesc")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
