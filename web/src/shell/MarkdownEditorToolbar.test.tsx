...
describe("MarkdownEditorToolbar auto-save status", () => {
  ...
  it("shows toolbar button tooltips in Chinese when locale is zh-CN", () => {
    setLocale("zh-CN");
    renderToolbar();
    expect(screen.getByTitle("粗体（⌘B）")).toBeInTheDocument();
    expect(screen.getByTitle("斜体（⌘I）")).toBeInTheDocument();
    expect(screen.getByTitle("撤销（⌘Z）")).toBeInTheDocument();
    setLocale("en");
  });
});
