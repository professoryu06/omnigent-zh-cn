  ...
  return (
    ...
    >
      ...
      <header className="flex shrink-0 items-center justify-between border-border border-b px-4 py-3">
        <h2 className="font-medium text-sm">{t("logs.executionLogs")}</h2>
        <Button type="button" variant="ghost" size="icon-sm" aria-label={t("logs.close")} onClick={onClose}>
...
function buildLogEntries(conversationId: string, children: ChildSessionInfo[]): LogEntry[] {
  const main: LogEntry = {
    ...
    label: translate("logs.mainEntry"),
...
function SessionItemsList({ sessionId }: { sessionId: string }) {
  ...
  if (isLoading) {
    return <div className="text-muted-foreground text-xs">{t("logs.loading")}</div>;
  }
  if (error) {
    return <div className="text-destructive text-xs">{t("logs.loadFailed", { message: error.message })}</div>;
  }
  if (items.length === 0) {
    return <div className="text-muted-foreground text-xs">{t("logs.noItems")}</div>;
  }
  return (
    <div
      ref={scrollRootRef}
    ...
    >
      ...
      {hasNextPage && (
        <div ref={sentinelRef} className="py-2 text-center text-muted-foreground text-xs">
          {isFetchingNextPage ? t("logs.loadingMore") : ""}
        </div>
      )}
    </div>
  );
