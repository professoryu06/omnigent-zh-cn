...
describe("AddAgentDialog", () => {
  ...
  it("shows Chinese dialog title when locale is zh-CN", () => {
    setLocale("zh-CN");
    renderDialog();
  ...
  it("keeps the no-agent command inside a complete Chinese sentence", () => {
    setLocale("zh-CN");
    mockAgents([]);
    renderDialog();

    expect(screen.getByTestId("add-agent-empty")).toHaveTextContent(
      "此服务器上无可用智能体。请使用 omnigent server --agent 注册一个。",
    );
  });
});
