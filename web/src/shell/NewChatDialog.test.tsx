...
describe("NewChatDialog zh-CN", () => {
  afterEach(() => setLocale("en"));
  it("renders the hero heading in Chinese", () => {
    setLocale("zh-CN");
    renderLanding();
...
describe("NewChatDialog zh-CN new sandbox", () => {
  afterEach(() => setLocale("en"));
  it("renders disabled sandbox label and help aria-label in Chinese when managed sandboxes are unavailable", async () => {
    setLocale("zh-CN");
    setOmnigentHostConfig({
      docsLinks: { newSandbox: "Managed sandboxes are disabled in this workspace." },
    });
    renderLanding();
    // Open the host chip dropdown
    fireEvent.pointerDown(screen.getByTestId("new-chat-landing-host-chip"), { button: 0 });

    // The disabled sandbox row should be visible with Chinese text
    const disabledRow = screen.getByTestId("new-chat-landing-sandbox-option-disabled");
    expect(disabledRow).toBeTruthy();
    expect(disabledRow.textContent).toContain("新建沙箱");

    // The help button should have the Chinese aria-label
    const helpBtn = screen.getByLabelText("为什么新建沙箱不可用");
    expect(helpBtn).toBeTruthy();

    // Clicking it should not switch to sandbox (not selectable)
    fireEvent.click(disabledRow);
    expect(screen.queryByTestId("new-chat-landing-repo-chip")).toBeNull();
    // Host chip should still read its original label, not sandbox
    expect(screen.getByTestId("new-chat-landing-host-chip").textContent).not.toContain("新建沙箱");
  });
});