...
function formatCommentTime(createdAt: number): string {
  ...
  if (date.toDateString() === now.toDateString()) return `${time}${translate("comment.today")}`;
  const yesterday = new Date(now);
  yesterday.setDate(now.getDate() - 1);
  if (date.toDateString() === yesterday.toDateString()) return `${time}${translate("comment.yesterday")}`;
  ...
  return (
    ...
    >
      ...
      <div className="flex h-11 shrink-0 items-center justify-between px-3 border-b border-border">
        <span className="text-xs font-semibold">{t("comment.comments")}</span>
        {tab === "open" && (
          <Button
            type="button"
            variant="outline"
      ...
      <div className="flex shrink-0 border-b border-border">
        {TABS.map((tabVal) => {
          const count = tabVal === "open" ? comments.length : addressedComments.length;
          return (
            <button
              key={tabVal}
              type="button"
            ...
            >
              {tabVal === "open" ? t("comment.open") : t("comment.addressed")}
              {count > 0 && (
            ...
            <div className="space-y-2 border-b border-border px-3 py-2">
              {activeSelection.anchor_content && (
                <div className="truncate rounded bg-muted/40 px-2 py-1 font-mono text-[10px] text-muted-foreground">
                  <span className="text-foreground/60">{t("comment.selection")}</span>
...
function CommentCard({
  ...
  const statusLabel = c.status === "addressed" ? t("comment.addressedStatus") : null;
      ...
      {editing ? (
        ...
        <div className="space-y-1">
          ...
          {(clamped || expanded) && (
            ...
            >
              {expanded ? t("comment.showLess") : t("comment.showMore")}
            </button>
          )}
        </div>
      )}
          ...
          <div className="flex min-w-0 flex-col gap-0.5">
            <div className="flex min-w-0 items-center gap-1.5">
              <span
                ...
                style={avatarStyle(c.created_by ?? t("comment.you"))}
              >
                {(c.created_by ?? t("comment.you"))[0].toUpperCase()}
              </span>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <span className="truncate text-[11px] text-muted-foreground">
                      {c.created_by ?? t("comment.you")}
                    </span>
                  </TooltipTrigger>
                  <TooltipContent>{c.created_by ?? t("comment.you")}</TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
          ...
          {(onEdit || onDelete || onCopyLink) && (
            <div className="flex shrink-0 items-center gap-2 mr-0.5">
              ...
              {onCopyLink && (
                <button
                  type="button"
                  aria-label={t("comment.copyLinkToComment")}
                  className="cursor-pointer text-[11px] text-muted-foreground transition-colors hover:text-foreground"
                  onClick={(e) => {
                    e.stopPropagation();
                    onCopyLink();
